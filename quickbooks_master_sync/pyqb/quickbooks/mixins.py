...
class ListMixin:
	...
	def where(cls, where_clause="", start_position="", max_results=""):
		...
		select = f"SELECT * FROM {cls.qbo_object_name} {where_clause}{start_position}{max_results}"
...
class QuickbooksPdfDownloadable:
	...
	def download_pdf(self):
		if self.Id and self.Id > 0:
			...
			raise QuickbooksException(f"Cannot download {self.qbo_object_name} when no Id is assigned")
