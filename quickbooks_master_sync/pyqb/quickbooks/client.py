...
class QuickBooks(object):
    ...
    def create_session(self):
        ...
        if self.access_token:
            ...
            class OAuth2SessionWrapper:
                ...
                def request(
                    self, method, url, use_oauth=True, company_id=None, *args, **kwargs
                ):
                    """
    ...
    def get_authorize_url(self, force_company_selection=False):
        ...
        if not self.consumer_key:
            raise QuickbooksException(
                "Consumer Key (Client ID) is required for OAuth 2.0", 10000
            )
        ...
        authorize_url = "{0}?{1}".format(self.authorize_url, query_string)
    ...
    def get_access_tokens(self, authorization_code):
        ...
        if not authorization_code:
            raise QuickbooksException(
                "Authorization code is required for OAuth 2.0 token exchange", 10000
            )

        if not self.consumer_key or not self.consumer_secret:
            raise QuickbooksException(
                "Consumer key and secret are required for OAuth 2.0", 10000
            )

        # OAuth 2.0 token exchange - use Basic Auth with client_id:client_secret
        credentials = "{0}:{1}".format(self.consumer_key, self.consumer_secret)
        ...
        try:
            response = requests.post(
                self.access_token_url, data=token_data, headers=headers
            )
            ...
            if response.status_code != 200:
                ...
                raise QuickbooksException(
                    "Failed to exchange authorization code for tokens. Status: {0}, Response: {1}".format(
            ...
            # traceback.print_exc()
            raise QuickbooksException(
                "Network error during token exchange: {0}".format(str(e)), 10000
            )
    ...
    def make_request(
        self, request_type, url, request_body=None, content_type="application/json"
    ):
        ...
        if fault:
            ...
            if isinstance(errors, list):
                for error in errors:
                    ...
                    if "Authorization Failure" in message or "Authorization" in message or "Authentication" in message:
                        is_auth_error = True
                        break
        ...
        if req.status_code != httplib.OK:
            ...
            if fault:
                ...
                raise QuickbooksException(
                    "HTTP {0}: {1}".format(req.status_code, req.text),
                    req.status_code,
                )
        ...
        if req.status_code == 401:
            if self._attempt_token_refresh():
                ...
                if req.status_code == 401:
                    ...
                    raise QuickbooksException(
                        "Authorization Failure: Token refresh succeeded but authorization still failed. Your QuickBooks connection may have expired. Response: {0}. Please reconnect to QuickBooks by clicking 'Reconnect to QuickBooks' in QuickBooks Settings.".format(req.text),
                        401
                    )
                ...
                if retry_fault:
                    ...
                    if isinstance(retry_fault, dict):
                        retry_errors = retry_fault.get("Error") or retry_fault.get("error") or retry_fault.get("errors") or []
                    ...
                    if retry_is_auth_error:
                        ...
                        raise QuickbooksException(
                            "Authorization Failure: Token refresh succeeded but authorization still failed. Your QuickBooks connection may have expired. Response: {0}. Please reconnect to QuickBooks by clicking 'Reconnect to QuickBooks' in QuickBooks Settings.".format(json.dumps(result)),
                            401
                        )
            else:
                # Token refresh failed - raise clear error
                raise QuickbooksException(
                    "Authorization Failure: Unable to refresh access token. Response: {0}. Please reconnect to QuickBooks by clicking 'Reconnect to QuickBooks' in QuickBooks Settings.".format(req.text),
                    401
                )
            ...
            if is_auth_error and self._attempt_token_refresh():
                ...
                if retry_fault:
                    ...
                    if retry_is_auth_error:
                        # Still authorization error after refresh - tokens are invalid/expired
                        raise QuickbooksException(
                            "Authorization Failure: Token refresh succeeded but authorization still failed. Response: {0}. Your QuickBooks connection may have expired (refresh token expired after 100 days). Please reconnect to QuickBooks by clicking 'Reconnect to QuickBooks' in QuickBooks Settings.".format(json.dumps(result)),
                            401
                        )
                ...
                if req.status_code == 401:
                    raise QuickbooksException(
                        "Authorization Failure: Token refresh succeeded but authorization still failed (HTTP 401). Response: {0}. Your QuickBooks connection may have expired. Please reconnect to QuickBooks by clicking 'Reconnect to QuickBooks' in QuickBooks Settings.".format(req.text),
                        401
                    )
                ...
                # Token refresh failed - raise clear error
                raise QuickbooksException(
                    "Authorization Failure: Unable to refresh access token. Response: {0}. Please reconnect to QuickBooks by clicking 'Reconnect to QuickBooks' in QuickBooks Settings.".format(json.dumps(result)),
                    401
                )
    ...
    def handle_exceptions(self, results):
        ...
        if isinstance(fault_dict, dict):
            errors = (
                fault_dict.get("Error")
                or fault_dict.get("error")
                or fault_dict.get("errors")
                or []
            )
        ...
        for error in errors:
            message = (
                error.get("Message") or error.get("message") or "QuickBooks API Error"
            )
    ...
    def download_pdf(self, qbbo, item_id):
        ...
        response = self.session.request(
            "GET", url, True, self.company_id, headers=headers
        )
    ...
    def _attempt_token_refresh(self):
        ...
        if not refresh_token:
            ...
            try:
                import frappe
                frappe.log_error(
                    message="Token refresh failed: No refresh token available. refresh_token='{0}', access_token_secret='{1}'".format(
                        self.refresh_token or "None",
                        self.access_token_secret or "None"
                    ),
        ...
        try:
            ...
            if resp.status_code != 200:
                ...
                try:
                    import frappe
                    frappe.log_error(
                        message="Token refresh API call failed: HTTP {0}, Response: {1}".format(
                            resp.status_code, resp.text[:500]
                        ),
                        title="QuickBooks Token Refresh - API Error"
                    )
                except:
                    print("Token refresh API call failed: HTTP {0}".format(resp.status_code))
                return False
            ...
            if not new_access:
                ...
                try:
                    import frappe
                    frappe.log_error(
                        message="Token refresh response missing access_token. Response: {0}".format(
                            str(token_response)[:500]
                        ),
            ...
            try:
                ...
                try:
                    import frappe
                    frappe.log_error(
                        title="QuickBooks Token Refresh - Save Warning"
                    )
                except:
                    print(f"Token refreshed successfully but failed to save to database: {str(save_error)}")
            
            # Log successful refresh
            try:
                import frappe
                frappe.log_error(
                    message="Token refreshed successfully. New access token length: {0}".format(len(new_access)),
