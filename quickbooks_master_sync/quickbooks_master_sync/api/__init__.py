...
from frappe.utils import cint, get_url, flt
import time
...
from quickbooks_master_sync.quickbooks_master_sync.utils.logging import qb_log_error, qb_log_status, qb_log_exception
...
def sync_from_quickbooks_to_erp(quickbooks_settings):
    ...
    try:
        _publish_realtime_progress("Analyzing QuickBooks...", "running", "Fetching company info and initializing cache")
        ...
        # 1. Company
        try:
            _publish_realtime_progress("Syncing Company...", "running", "Synchronizing QuickBooks company details and foundations")
        ...
        # 2. Master Data
        try:
            _publish_realtime_progress("Syncing Customers...", "running", "Synchronizing QuickBooks customers")
            sync_customers(quickbooks_obj)
        except Exception as e:
            qb_log_exception(method="sync_customers", err=e, module="api")

        try:
            _publish_realtime_progress("Syncing Suppliers...", "running", "Synchronizing QuickBooks suppliers")
            sync_suppliers(quickbooks_obj)
        except Exception as e:
            qb_log_exception(method="sync_suppliers", err=e, module="api")

        try:
            _publish_realtime_progress("Syncing Employees...", "running", "Synchronizing QuickBooks employees")
            create_Employee(quickbooks_obj)
        except Exception as e:
            qb_log_exception(method="create_Employee", err=e, module="api")

        try:
            _publish_realtime_progress("Syncing Products...", "running", "Synchronizing QuickBooks items and categories")
            create_Item(quickbooks_obj)
        except Exception as e:
            qb_log_exception(method="create_Item", err=e, module="api")

        try:
            _publish_realtime_progress("Syncing Opening Balances...", "running", "Synchronizing QuickBooks opening balances")
    ...
    frappe.db.set_single_value(
        "Quickbooks Settings", "last_sync_datetime", frappe.utils.now()
    )
...
def _create_quickbooks_client(quickbooks_settings):
    ...
    if existing_instance:
        if existing_instance.company_id != quickbooks_settings.realm_id or \
           existing_instance.access_token != quickbooks_settings.access_token:
    ...
    try:
        ...
        if isinstance(list_full_response_only, str):
            list_full_response_only = (
                list_full_response_only.lower() == "true"
                or list_full_response_only == "1"
            )
        ...
        for filename in all_files:
            try:
                ...
                is_debug_log = (
                    len(parts) >= 3 and parts[1] == "debug" and parts[2] == "logs"
                )
                ...
                if is_debug_log:
                    ...
                    if "full" in parts:
                        ...
                        if len(parts) >= 5 and full_idx > 4:
                            module_parts = parts[
                                4:full_idx
                            ]  # Everything after company until "full"
                        ...
                        if len(parts) >= 5:
                            module_parts = parts[
                                4:-2
                            ]  # Everything after company until last 2 parts
                            data_name = f"debug_logs_{'_'.join(module_parts)}"
                        else:
                            data_name = "debug_logs"
                else:
                    ...
                    start_idx = (
                        2 if company_abbr else 1
                    )  # Skip "qb" and company if present
        ...
        if sort_by == "size":
            ...
            debug_files.sort(
                key=lambda x: x["data_name"].lower(), reverse=reverse_order
            )
        ...
        return {
            ...
            "total_size_mb": round(
                sum(f["size"] for f in debug_files) / (1024 * 1024), 2
            ),
        ...
        frappe.log_error(
            f"Error getting debug files: {str(e)}", "get_quickbooks_debug_files"
        )
        return {
            "success": False,
            "error": str(e),
...
def get_quickbooks_debug_file_content(filename):
    ...
    try:
        ...
        if not filename.startswith("qb_") or (
            ...
            frappe.throw(
                _(
                    "Invalid filename. Only QuickBooks debug files (.json/.log) are allowed."
                )
            )
                    ...
                    if isinstance(value, list):
                        ...
                        for nested_value in value.values():
                            if isinstance(nested_value, list):
                                max_list_length = max(
                                    max_list_length, len(nested_value)
                                )
        ...
        frappe.log_error(
            f"Error reading debug file {filename}: {str(e)}",
...
def delete_quickbooks_debug_file(filename):
    ...
    try:
        ...
        frappe.log_error(
            f"Error deleting debug file {filename}: {str(e)}",
...
def clear_all_quickbooks_debug_files(company_filter=None):
    ...
    try:
        ...
        frappe.log_error(
            f"Error clearing debug files: {str(e)}", "clear_all_quickbooks_debug_files"
        )
...
def compare_quickbooks_erpnext_balances(company_name=None, as_of_date=None, tolerance=0.01):
    ...
    try:
        ...
        import traceback
        frappe.log_error(f"Error in compare_quickbooks_erpnext_balances: {str(e)}\n{traceback.format_exc()}", "compare_quickbooks_erpnext_balances")
        return {"success": False, "error": str(e)}

@frappe.whitelist()
def compare_party_balances(party_type, company_name=None, as_of_date=None, tolerance=0.01):
    from quickbooks_master_sync.quickbooks_master_sync.compare_balances import compare_party_balances as compare_parties
    ...
    try:
        ...
        import traceback
        frappe.log_error(f"Error in compare_party_balances: {str(e)}\n{traceback.format_exc()}", "compare_party_balances")
        return {"success": False, "error": str(e)}

@frappe.whitelist()
def compare_quickbooks_erpnext_date_wise_balances(erp_account, qb_id, start_date, end_date, company_name=None, tolerance=0.01):
    ...
    try:
        ...
        import traceback
        frappe.log_error(f"Error in compare_quickbooks_erpnext_date_wise_balances: {str(e)}\n{traceback.format_exc()}", "compare_quickbooks_erpnext_date_wise_balances")
        return {"success": False, "error": str(e)}
