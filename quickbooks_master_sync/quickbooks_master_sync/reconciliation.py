...
def get_company_from_filename(filename):
    ...
    if potential_company:
        ...
        company_abbr = frappe.db.get_value(
            "Company", {"name": potential_company}, "abbr"
        )
...
def reconcile_invoices(records, results, entity_type, company_abbr=None):
    ...
    id_field = (
        ...
        if entity_type == "Invoice"
        else "quickbooks_purchase_invoice_id"
    )
    ...
    for record in records:
        ...
        if company_abbr:
            ...
            exists = frappe.db.get_value(
                doctype, filters, ["name", "grand_total"], as_dict=True
            )
...
def reconcile_items(records, results, company_abbr=None):
    ...
    for record in records:
        ...
        if company_abbr:
            exists = frappe.db.get_value(
                "Item", {"quickbooks_item_id": f"{company_abbr}-{qb_id}"}, "name"
            )
    ...
    for record in records:
        ...
        if "TxnDate" in record:
            ...
            if not exists:
                ...
                results["matched"].append(
                    {"qb_id": qb_id, "name": name, "erp_name": exists}
                )
    return results
...
def reconcile_transfers(records, results, company_abbr=None):
    ...
    for record in records:
        ...
        if not exists:
            results["missing_in_erpnext"].append(
                {"qb_id": qb_id, "txn_date": txn_date, "total_amt": amount}
            )
        else:
...
def reconcile_accounts(records, results, company_abbr=None):
    ...
    for record in records:
        ...
        if company_abbr:
            exists = frappe.db.get_value(
                "Account", 
                {"quickbooks_account_id": f"{company_abbr}-{qb_id}"}, 
                "name"
            )
