...
Usage:
    from quickbooks_to_erpnext.compare_balances import compare_account_balances
...
from .sync.sync_utils import query_with_pagination, _get_quickbooks_company, _dbg, save_qb_data_to_json
from .utils.logging import qb_log_status, qb_log_error
...
def _fetch_quickbooks_balances(quickbooks_obj, company_name, as_of_date):
    ...
    try:
        ...
        report_url = quickbooks_obj.api_url + "/company/{0}/reports/TrialBalance".format(quickbooks_obj.company_id)
...
def _fetch_erpnext_balances(company_name, as_of_date):
    ...
    accounts = frappe.get_all(
        ...
        fields=["name", "account_name", "quickbooks_account_id", "account_type", "root_type", "account_currency"]
    )
    ...
    for acc in accounts:
        ...
        company_abbr, qb_id = parse_qb_id(stored_qb_id)  # Extract raw ID for comparison with QuickBooks
        if not qb_id:
            _dbg("compare_balances:parse_failed", {
                "account": acc["name"],
                "stored_qb_id": stored_qb_id,
                "company": company_name
            })
        ...
        balances[qb_id] = {
            ...
            "erp_balance": balance,
            "currency": acc.get("account_currency") or frappe.get_cached_value("Company", company_name, "default_currency"),
            "stored_qb_id": stored_qb_id,  # Keep original for debugging
            "parsed_qb_id": qb_id  # Parsed raw ID
        }

    _dbg("compare_balances:erp_fetched", {
...
def _calculate_erpnext_balance(account, company, as_of_date):
    ...
    try:
        ...
        gl_entries = frappe.db.sql("""
            ...
            AND is_cancelled = 0
            AND docstatus = 1
        """, (account, company, as_of_date), as_dict=True)
        ...
        entry_count = gl_entries[0].get("entry_count", 0)
        ...
        _dbg("compare_balances:calc_balance_error", {
            "account": account,
            "error": str(e)
        })
        return 0.0
...
def _compare_balances(qb_balances, erp_balances, tolerance):
    ...
    _dbg("compare_balances:compare_start", {
    ...
    for qb_id, qb_data in qb_balances.items():
        if qb_id in erp_balances:
            ...
            if abs(qb_balance) > 0.001 or (has_subaccounts and abs(qb_data.get("qb_balance_with_subs", 0)) > 0.001):
                qb_only.append({
                    "qb_id": qb_id,
                    "qb_name": qb_data["qb_name"],
                    "qb_balance": qb_balance,
                    ...
                    "note": "Parent account with balance - may need to be synced to ERPNext" if has_subaccounts else None
                })
    ...
    def add_to_category(root_type, qb_val, erp_val):
        ...
        if not root_type:
            ...
            target = root_type
             ...
             if "Asset" in root_type: target = "Asset"
             elif "Liability" in root_type: target = "Liability"
             elif "Equity" in root_type: target = "Equity"
             elif "Income" in root_type: target = "Income"
             elif "Expense" in root_type: target = "Expense"
             else: target = "Uncategorized"
    ...
    result = {
        ...
        # matched, mismatched, qb_only, erp_only are intentionally excluded (premium-gated)
    }
...
def _log_comparison_results(comparison, company_name, as_of_date):
    ...
    if summary["mismatched_accounts"] > 0 or summary["qb_only_accounts"] > 0 or summary["erp_only_accounts"] > 0:
        # Has discrepancies
        message = _(
...
@frappe.whitelist()
def compare_date_wise_balances(company_name=None, start_date=None, end_date=None, erp_account=None, qb_id=None, tolerance=0.01):
    """
    ...
    qb_totals = _fetch_quickbooks_daily_totals(quickbooks_obj, start_date, end_date, qb_id, root_type=root_type)
...
def _fetch_erpnext_daily_totals(company, start_date, end_date, erp_account=None):
    ...
    for row in raw_data:
        ...
        totals[date_str] = {
            "erp_debit": flt(row["total_debit"]),
            "erp_credit": flt(row["total_credit"])
        }
    return totals
...
def _fetch_quickbooks_daily_totals(quickbooks_obj, start_date, end_date, qb_account_id=None, root_type=None):
    ...
    try:
        report_url = quickbooks_obj.api_url + "/company/{0}/reports/GeneralLedger".format(quickbooks_obj.company_id)
        ...
        for i, col in enumerate(columns_meta):
            ...
            if col_type == "date" or "date" in label or col_key == "tx_date":
                if col_indices["date"] == -1: col_indices["date"] = i
                ...
                if col_indices["amount"] == -1: col_indices["amount"] = i
            ...
            for row in row_list:
                ...
                if "ColData" in row:
                    ...
                    if date_val:
                        ...
                        if debit_val == 0 and credit_val == 0 and col_indices["amount"] != -1 and len(col_data) > col_indices["amount"]:
                        ...
                        if has_amount:
                            _dbg("compare_balances:skipped_row", {"row_type": row.get("type"), "col_data": col_data})
...
def _fetch_erpnext_party_balances(company, party_type, as_of_date):
    ...
    gl_data = frappe.db.sql(f"""
        ...
        AND is_cancelled = 0
        GROUP BY party
    """, (company, party_type, list(party_map.keys()), as_of_date), as_dict=True)
    ...
    for row in gl_data:
        ...
        balances[qb_id] = {
            "erp_name": row.party,
            "erp_balance": flt(row.net_balance),
            "qb_id": qb_id
        }
...
def get_quickbooks_journal_details(company_name, date, account_name, amount, qb_id=None):
    ...
    report_url = quickbooks_obj.api_url + "/company/{0}/reports/JournalReport".format(quickbooks_obj.company_id)
    ...
    try:
        ...
        for i, row in enumerate(all_data_rows):
            ...
            if acc_match and (abs(row_debit - target_amount) < 0.01 or abs(row_credit - target_amount) < 0.01):
...
def create_journal_entry_from_qb(rows, company_name):
    ...
    for row in rows:
        ...
        if qb_account_id:
            ...
            erp_account = frappe.db.get_value("Account", {
                "company": company_name,
        ...
        if not erp_account:
            ...
            account_name_clean = qb_account_name.split(" - ")[-1] if " - " in qb_account_name else qb_account_name
            accs = frappe.get_all("Account", filters={"company": company_name, "is_group": 0}, fields=["name", "account_name"])
            for acc in accs:
        ...
        if not erp_account:
            frappe.throw(_("Could not find a matching ERPNext account for QuickBooks account: {0} (ID: {1})").format(qb_account_name, qb_account_id or 'N/A'))
