...
def login_via_oauth2(realmId, auth_code):
	...
	callback_url = get_url('/api/method/quickbooks_master_sync.quickbooks_master_sync.doctype.quickbooks_settings.quickbooks_settings.First_callback')
	...
	try:
		from quickbooks_master_sync.quickbooks_master_sync.sync.sync_company import sync_quickbooks_company_with_client
		...
		if not quickbooks.company_id:
			quickbooks.company_id = realmId
		sync_result = sync_quickbooks_company_with_client(quickbooks)
		...
		frappe.log_error(
			message=f"Failed to sync company after QuickBooks connection: {str(e)}\n\nTraceback:\n{frappe.get_traceback()}",
...
def quickbooks_authentication_popup(consumer_key, consumer_secret, force_company_selection=False):
	...
	force_company_selection = cint(force_company_selection) if isinstance(force_company_selection, str) else bool(force_company_selection)
	...
	quickbooks_settings.authorize_url = quickbooks.get_authorize_url(force_company_selection=force_company_selection)
...
def clear_quickbooks_connection():
	...
	return {
		"status": "success",
		"message": _("QuickBooks connection cleared. When you reconnect, you will be able to select a different company.")
	}
