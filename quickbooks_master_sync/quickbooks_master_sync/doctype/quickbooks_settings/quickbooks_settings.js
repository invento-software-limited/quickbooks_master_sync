...
frappe.ui.form.on('Quickbooks Settings', {
	refresh: function (frm) {
		...
		if (!frm.doc.__islocal && frm.doc.enable_quickbooks_online === 1) {
			frm.add_custom_button(__('Sync Data to Quickbooks'), function () {
				cur_frm.cscript.sync_data_to_qb(frm);
			}, __('Actions'));
		}
		// Add "Compare Balances" button
		if (!frm.doc.__islocal && frm.doc.enable_quickbooks_online === 1) {
			frm.add_custom_button(__('Compare Balances'), function () {
		...
		if (!frm.doc.__islocal) {
			frm.add_custom_button(__('Delete Company Data'), function () {
		...
		if (!frm.doc.__islocal && frm.doc.realm_id && frm.doc.realm_id != "") {
			frm.add_custom_button(__('Reconnect to QuickBooks'), function () {
	...
	setup: function (frm) {
		...
		let sync_buttons = ["customer_sync", "account_sync", "supplier_sync", "employee_sync", "product_sync", "company_sync", "payment_method_sync"];
...
if (cur_frm.fields_dict["company_settings"]) {
	cur_frm.fields_dict["company_settings"].grid.get_field("warehouse").get_query = function (doc, cdt, cdn) {
...
cur_frm.cscript.connect_to_qb = function () {
	...
	cur_frm.cscript.sync_data_to_qb = function (frm) {
		...
		if (!cur_frm.doc.__islocal && cur_frm.doc.enable_quickbooks_online === 1) {
			// Validate that at least one company has settings configured
			let warnings = [];
			if (!cur_frm.doc.company_settings || cur_frm.doc.company_settings.length === 0) {
				...
				if (!has_settings) {
					warnings.push('Price Lists (configure at least one company with Selling/Buying Price List)');
				}
			}
					...
					if (data && data.step) {
						...
						if (data.status === 'completed' || (data.step && (
							...
							setTimeout(function () {
								...
								if (data.status === 'completed' || data.step === 'Sync Completed') {
									frappe.show_alert({
									...
									frappe.msgprint({
										title: __('Sync Error'),
										message: data.message || __('Sync failed. Please check the error log.'),
										indicator: 'red'
									});
								}
				...
				return frappe.call({
					...
					callback: function (r) {
						...
						if (r.message && r.message.job_queued === true) {
							...
							update_sync_progress('Sync Queued', 'running', 'Sync job queued and running in background...');
						...
						if (r.message && r.message.success === false) {
							update_sync_progress('Sync Failed', 'error', r.message.message || 'Sync failed');
							setTimeout(function () {
								if (progress_dialog) {
									progress_dialog.hide();
								}
								frappe.msgprint({
									title: __('Sync Error'),
									message: r.message.message || __('Sync failed. Please check the error log.'),
									indicator: 'red'
								});
							}, 1000);
						} else {
							// Immediate completion (shouldn't happen with background jobs)
							update_sync_progress('Sync Completed', 'completed', 'All data synced successfully');
							setTimeout(function () {
								if (progress_dialog) {
									progress_dialog.hide();
								}
			...
			if (warnings.length > 0) {
				frappe.warn(
					...
					__('The following fields are recommended but not configured: {0}. Sync may use fallback values. Continue?',
						[warnings.join(', ')]),
					start_sync,
					__('Continue'),
					true
...
function update_sync_progress(step_name, status, message) {
	...
	if (existing_step.length === 0) {
			...
			<div class="sync-progress-item" id="${step_id}" style="padding: 10px; border-bottom: 1px solid #ddd;">
				<div style="display: flex; align-items: center;">
					...
					<div style="flex: 1;">
						...
						${message ? `<div style="font-size: 11px; color: #8d99a6; margin-top: 2px;">${frappe.utils.escape_html(message)}</div>` : ''}
					</div>
				</div>
			</div>
		`;
		...
		if (status === 'completed') {
			icon.removeClass('fa-spinner fa-spin').addClass('fa-check-circle').css('color', '#28a745');
		} else if (status === 'error') {
			icon.removeClass('fa-spinner fa-spin').addClass('fa-times-circle').css('color', '#dc3545');
		}
		...
		if (message) {
			...
			if (message_div.length) {
				...
				existing_step.find('div[style*="font-weight: 500"]').after(
					`<div style="font-size: 11px; color: #8d99a6; margin-top: 2px;">${frappe.utils.escape_html(message)}</div>`
				);
			}
		}
	}
...
cur_frm.cscript.customer_sync = function () {
	if (!cur_frm.doc.__islocal && cur_frm.doc.enable_quickbooks_online === 1) {
		...
		msgprint(__("Enable QuickBooks Online and save the settings first."));
	}
};
				...
				if (data.status === 'completed' || (data.step && (
					...
					setTimeout(function () {
						...
						if (data.status === 'completed' || data.step.includes('Completed')) {
							frappe.show_alert({
								message: __("{0} sync completed successfully!", [resource_name]),
								indicator: 'green'
							}, 5);
						} else {
							frappe.msgprint({
...
cur_frm.cscript.fixed_asset_sync = function () {
	return _start_sync_with_progress("quickbooks_master_sync.quickbooks_master_sync.api.sync_quickbooks_fixed_assets_only", "Fixed Asset");
};

cur_frm.cscript.account_sync = function () {
	return _start_sync_with_progress("quickbooks_master_sync.quickbooks_master_sync.api.sync_quickbooks_accounts_only", "Account");
};

cur_frm.cscript.employee_sync = function () {
	return _start_sync_with_progress("quickbooks_master_sync.quickbooks_master_sync.api.sync_quickbooks_employees_only", "Employee");
};

cur_frm.cscript.supplier_sync = function () {
	return _start_sync_with_progress("quickbooks_master_sync.quickbooks_master_sync.api.sync_quickbooks_suppliers_only", "Supplier");
};

cur_frm.cscript.product_sync = function () {
	return _start_sync_with_progress("quickbooks_master_sync.quickbooks_master_sync.api.sync_quickbooks_products_only", "Product");
};

cur_frm.cscript.sales_invoice_sync = function () {
	return _start_sync_with_progress("quickbooks_master_sync.quickbooks_master_sync.api.sync_quickbooks_sales_invoices_only", "Sales Invoice");
};

cur_frm.cscript.credit_memo_sync = function () {
	return _start_sync_with_progress("quickbooks_master_sync.quickbooks_master_sync.api.sync_quickbooks_credit_memos_only", "Credit Memo");
};

cur_frm.cscript.vendor_credit_sync = function () {
	return _start_sync_with_progress("quickbooks_master_sync.quickbooks_master_sync.api.sync_quickbooks_vendor_credits_only", "Vendor Credit");
};

cur_frm.cscript.purchase_invoice_sync = function () {
	return _start_sync_with_progress("quickbooks_master_sync.quickbooks_master_sync.api.sync_quickbooks_purchase_invoices_only", "Purchase Invoice");
};

cur_frm.cscript.payment_entry_sync = function () {
	return _start_sync_with_progress("quickbooks_master_sync.quickbooks_master_sync.api.sync_quickbooks_payment_entries_only", "Payment Entry");
};

cur_frm.cscript.payment_method_sync = function () {
	return _start_sync_with_progress("quickbooks_master_sync.quickbooks_master_sync.api.sync_quickbooks_payment_methods_only", "Payment Method");
};

cur_frm.cscript.journal_entry_sync = function () {
	return _start_sync_with_progress("quickbooks_master_sync.quickbooks_master_sync.api.sync_quickbooks_journal_entries_only", "Journal Entry");
};

cur_frm.cscript.deposits_sync = function () {
	return _start_sync_with_progress("quickbooks_master_sync.quickbooks_master_sync.api.sync_quickbooks_deposits_only", "Deposit");
};

cur_frm.cscript.transfers_sync = function () {
	return _start_sync_with_progress("quickbooks_master_sync.quickbooks_master_sync.api.sync_quickbooks_transfers_only", "Transfer");
};

cur_frm.cscript.credit_card_payment_sync = function () {
	return _start_sync_with_progress("quickbooks_master_sync.quickbooks_master_sync.api.sync_quickbooks_credit_card_payments_only", "Credit Card Payment");
};

cur_frm.cscript.refund_receipt_sync = function () {
	return _start_sync_with_progress("quickbooks_master_sync.quickbooks_master_sync.api.sync_quickbooks_refund_receipts_only", "Refund Receipt");
};

cur_frm.cscript.sales_receipt_sync = function () {
	return _start_sync_with_progress("quickbooks_master_sync.quickbooks_master_sync.api.sync_quickbooks_sales_receipts_only", "Sales Receipt");
};
...
cur_frm.cscript.inventory_adjustment_sync = function () {
	return _start_sync_with_progress("quickbooks_master_sync.quickbooks_master_sync.api.sync_quickbooks_inventory_adjustments_only", "Inventory Adjustment");
};
		...
		fields: [
			...
			},
			{
				...
				click: function () {
					...
					run_balance_comparison(values.company, values.as_of_date, values.tolerance || 0.01, d);
				}
			}
		],
		primary_action_label: null
	...
	frappe.call({
		...
		callback: function (r) {
			if (r.message && r.message.success && r.message.result) {
				...
				frappe.msgprint({
					title: __('Error'),
					message: r.message && r.message.error ? r.message.error : __('Failed to compare balances. Please check the error log.'),
					indicator: 'red'
				});
			}
		},
		error: function (r) {
			frappe.msgprint({
				title: __('Error'),
				message: __('An error occurred while comparing balances. Please check the error log for details.'),
				indicator: 'red'
			});
		}
	});
...
function reconnect_to_quickbooks(frm) {
	frappe.confirm(
		__('Are you sure you want to disconnect from the current QuickBooks company and reconnect?<br><br>' +
			'<strong>Current Connection:</strong><br>' +
			'Realm ID: {0}<br><br>' +
			'This will clear your current connection. You will need to authorize again with QuickBooks to connect to a different company.', [frm.doc.realm_id || 'N/A']),
		function () {
			...
			frappe.call({
				...
				callback: function (r) {
					if (r.message && r.message.status === "success") {
						...
						setTimeout(function () {
							if ((frm.doc.consumer_key != null) && (frm.doc.consumer_secret != null) &&
								(frm.doc.consumer_key.trim() != "") && (frm.doc.consumer_secret.trim() != "")) {

								frappe.confirm(
									__('Connection cleared. Do you want to connect to QuickBooks now?<br><br>You will be able to select a different company.'),
									function () {
								...
								frappe.msgprint({
									...
									message: __("Connection cleared. Please enter your Consumer Key and Consumer Secret, then click 'Connect to QuickBooks' to reconnect."),
									indicator: 'blue'
								});
							}
						}, 1000);
	...
	frappe.call({
		...
		callback: function (r) {
			if (r.message && r.message.length > 0) {
				...
				let d = new frappe.ui.Dialog({
					...
					fields: [
						...
						},
						{
							...
							options: companies,
							reqd: 1,
							description: __('Select the company whose QuickBooks data you want to delete')
						},
						{
							fieldtype: 'HTML',
						...
						},
						{
							...
							default: 0,
							description: __('If checked, the Company document itself will also be deleted (along with all data)')
						},
						{
							fieldtype: 'Check',
							...
							default: 0,
							description: __('If checked, creates a Stock Reconciliation to zero out all stock before deletion')
						},
						{
							fieldtype: 'Section Break'
						},
						...
						},
						{
							...
							click: function () {
								...
								run_delete_company_data(values.company, true, values.delete_company || 0, values.clear_stock || 0, d);
							}
						},
						{
						...
						},
						{
							...
							click: function () {
								...
								frappe.confirm(
									__('Are you absolutely sure you want to permanently delete all QuickBooks data for company "{0}"? This action CANNOT be undone!', [values.company]),
									function () {
										run_delete_company_data(values.company, false, values.delete_company || 0, values.clear_stock || 0, d);
									}
								);
							}
						}
				...
				function check_default_company() {
					...
					if (warning_field && warning_field.wrapper) {
						if (default_company && selected_company && selected_company === default_company) {
							...
							$(warning_field.wrapper).html(`<div class="alert alert-warning">
								<strong>⚠️ Default Company Warning:</strong> The selected company "<strong>${frappe.utils.escape_html(selected_company)}</strong>" is your session default company.
				...
				function validate_company_match() {
					let selected_company = d.get_value('company');
					if (default_company && selected_company && selected_company !== default_company) {
						frappe.msgprint({
							title: __('Company Mismatch'),
							message: __('The selected company "<strong>{0}</strong>" does not match your session default company "<strong>{1}</strong>". Please select the correct company or change your session default company.', [
				...
				setTimeout(function () {
					if (d.fields_dict.company && d.fields_dict.company.wrapper) {
						$(d.fields_dict.company.wrapper).find('select').on('change', function () {
...
function run_delete_company_data(company, dry_run, delete_company, clear_stock, dialog) {
	...
	frappe.call({
		...
		error: function (r) {
			frappe.msgprint({
				title: __('Error'),
				message: __('An error occurred while processing the request. Please check the error log for details.'),
				indicator: 'red'
			});
		}
	});
