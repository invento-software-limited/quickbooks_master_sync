frappe.pages['quickbooks-balance-comparison'].on_page_load = function (wrapper) {
	...
	if (!comparison_data && frappe.route_options) {
		if (frappe.route_options.result) {
			try {
				comparison_data = typeof frappe.route_options.result === 'string'
	...
	let fields = [
		{
			...
			options: 'Company',
			reqd: 1,
			default: (frappe.route_options && frappe.route_options.company) ? frappe.route_options.company : frappe.defaults.get_user_default('company')
		},
		{
			...
			reqd: 1,
			default: (frappe.route_options && frappe.route_options.as_of_date) ? frappe.route_options.as_of_date : frappe.datetime.get_today()
		},
		{
			label: 'Tolerance',
			fieldtype: 'Float',
		...
		frappe.call({
			...
			callback: function (r) {
				if (r.message && r.message.success && r.message.result) {
					...
					frappe.msgprint({
						title: __('Error'),
						message: r.message && r.message.error ? r.message.error : __('Failed to compare balances.'),
						indicator: 'red'
					});
				}
			}
...
function render_comparison_results(page, data, is_date_wise = false, show_only_mismatched = false, account_info = null) {
	...
	if (!is_date_wise) {
			...
			<div class="row">
				<div class="col-md-12">
					<table class="table table-bordered table-sm">
						...
						<tbody>
							${render_category_row("Assets", summary.total_asset_qb, summary.total_asset_erp, summary.total_asset_diff, summary.tolerance)}
							${render_category_row("Liabilities", summary.total_liability_qb, summary.total_liability_erp, summary.total_liability_diff, summary.tolerance)}
							${render_category_row("Equity", summary.total_equity_qb, summary.total_equity_erp, summary.total_equity_diff, summary.tolerance)}
							${render_category_row("Income", summary.total_income_qb, summary.total_income_erp, summary.total_income_diff, summary.tolerance)}
							${render_category_row("Expenses", summary.total_expense_qb, summary.total_expense_erp, summary.total_expense_diff, summary.tolerance)}
							${(summary.total_uncategorized_qb || 0) !== 0 ? render_category_row("Uncategorized (QB Only)", summary.total_uncategorized_qb, summary.total_uncategorized_erp, summary.total_uncategorized_diff, summary.tolerance) : ''}
						</tbody>
					</table>
				</div>
			</div>
		...
		let diff_class = Math.abs(diff_total) > (summary.tolerance || 0.01) ? 'text-danger' : 'text-success';

		summary_html = `
			<div class="date-wise-header" style="margin-bottom: 20px;">
				<h5 class="text-muted">${__('Date-Wise Balance Comparison')}</h5>
				<h4 style="margin-bottom: 5px;">${account_info ? account_info.account_name : __('Unknown Account')}</h4>
				<p class="text-muted"><small>${__('QuickBooks ID')}: ${account_info ? account_info.qb_id : __('N/A')}</small></p>
			</div>
			...
			<div class="summary-cards" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; margin: 20px 0;">
				...
				<div class="card mismatched-days-card" style="cursor: pointer; ${show_only_mismatched ? 'border: 2px solid var(--red);' : ''}">
					<div class="card-body text-center">
						<h4 class="${mismatched_days > 0 ? 'text-danger' : 'text-success'}">${mismatched_days}</h4>
						<p class="text-muted mb-0">${show_only_mismatched ? __('Showing Mismatched Only') : __('Mismatched Days')}</p>
					</div>
				</div>
			</div>
		`;
	...
	function render_category_row(title, qb_val, erp_val, diff_val, tolerance) {
		let diff_class = Math.abs(diff_val || 0) > (tolerance || 0.01) ? 'text-danger font-weight-bold' : 'text-success';
	...
	` : `
		...
		<li class="nav-item">
			<a class="nav-link " data-toggle="tab" data-target="#matched-tab" href="javascript:void(0);" role="tab" aria-controls="matched-tab" aria-selected="true">Matched (<span id="matched-count">${summary.matched_accounts || 0}</span>)</a>
		</li>
		<li class="nav-item">
			<a class="nav-link" data-toggle="tab" data-target="#mismatched-tab" href="javascript:void(0);" role="tab" aria-controls="mismatched-tab" aria-selected="false">Mismatched (<span id="mismatched-count">${summary.mismatched_accounts || 0}</span>)</a>
		</li>
		<li class="nav-item">
			<a class="nav-link" data-toggle="tab" data-target="#qb-only-tab" href="javascript:void(0);" role="tab" aria-controls="qb-only-tab" aria-selected="false">QuickBooks Only (<span id="qb-only-count">${summary.qb_only_accounts || 0}</span>)</a>
		</li>
		<li class="nav-item">
			<a class="nav-link" data-toggle="tab" data-target="#erp-only-tab" href="javascript:void(0);" role="tab" aria-controls="erp-only-tab" aria-selected="false">ERPNext Only (<span id="erp-only-count">${summary.erp_only_accounts || 0}</span>)</a>
		</li>
	`;
	...
	function fetch_and_render_party(party_type, table_id) {
		...
		container.html(`<div class="text-center p-5"><div class="spinner-border text-primary" role="status"></div><p class="mt-2">Fetching ${party_type} balances...</p></div>`);
				...
				if (r.message) {
					table_configs[table_id] = {
						...
						columns: [
							...
							{ fieldname: 'qb_balance', label: __('QB Balance'), fieldtype: 'Currency' },
							{ fieldname: 'erp_balance', label: __('ERP Balance'), fieldtype: 'Currency' },
							{ fieldname: 'difference', label: __('Difference'), fieldtype: 'Currency' }
						],
						row_formatter: (row) => {
							let erp_link = row.erp_name && row.erp_name !== '-'
								? `<a href="/app/${party_type.toLowerCase()}/${encodeURIComponent(row.erp_name)}" target="_blank">${row.erp_name}</a>`
								: (row.erp_name || '-');
		...
		table_configs['date-wise-table'] = {
			...
			row_formatter: (row) => {
				...
				if (account_info && account_info.erp_account && account_info.company) {
					let gl_url = `/app/query-report/General%20Ledger?from_date=${row.date}&to_date=${row.date}&account=${encodeURIComponent(account_info.erp_account)}&company=${encodeURIComponent(account_info.company)}`;
				...
				return {
					...
					_matched: row.matched,
					actions: `<button class="btn btn-xs btn-default view-transactions-btn" data-date="${row.date}" title="${__('View Daily Transactions')}"><i class="fa fa-list"></i></button>`
				};
			},
	...
	function render_table_if_needed(table_id) {
		...
		try {
			render_datatable(table_id, config.data, config.columns, config.row_formatter, config.table_class);
	...
	setTimeout(() => {
		...
		page.body.find('#balance-tabs a[data-toggle="tab"]').off('click').on('click', function (e) {
			e.preventDefault();
			e.stopPropagation();
			let target = $(this).data('target');
			if (target && page.body.find(target).length) {
				// Show the target tab
				page.body.find('#balance-tabs a[data-toggle="tab"]').removeClass('active').attr('aria-selected', 'false');
	...
	page.body.off('click', '.mismatched-days-card').on('click', '.mismatched-days-card', function () {
		render_comparison_results(page, data, is_date_wise, !show_only_mismatched, account_info);
	});
	...
	page.body.off('click', '.compare-date-btn').on('click', '.compare-date-btn', function () {
		...
		let d = new frappe.ui.Dialog({
			...
			fields: [
				{ label: __('Company'), fieldtype: 'Data', fieldname: 'company', default: summary.company || frappe.defaults.get_user_default('company'), read_only: 1 },
				{ label: __('Account'), fieldtype: 'Data', fieldname: 'account_name', default: row_data.qb_name || row_data.erp_name, read_only: 1 },
				{ label: __('Start Date'), fieldtype: 'Date', fieldname: 'start_date', default: '2015-01-01', reqd: 1 },
				{ label: __('End Date'), fieldtype: 'Date', fieldname: 'end_date', default: frappe.datetime.get_today(), reqd: 1 },
				{ label: __('Show only differences'), fieldtype: 'Check', fieldname: 'show_only_mismatched', default: 1 },
				{ label: __('Tolerance'), fieldtype: 'Float', fieldname: 'tolerance', default: summary.tolerance || 0.01 }
			],
				...
				frappe.call({
					...
					callback: function (r) {
						if (r.message && r.message.success && r.message.result) {
							...
							render_comparison_results(page, { date_wise: r.message.result }, true, values.show_only_mismatched, {
							...
							page.set_primary_action(__('Back to Summary'), () => {
								...
								page.set_primary_action(__('Refresh'), () => show_comparison_form(page));
							});
						} else {
							frappe.msgprint({
								title: __('Error'),
								message: r.message && r.message.error ? r.message.error : __('Failed to compare date-wise balances.'),
								indicator: 'red'
							});
						}
					}
	...
	page.body.off('click', '.view-transactions-btn').on('click', '.view-transactions-btn', function () {
		...
		frappe.call({
			...
			args: {
				company_name: account_info.company || frappe.defaults.get_user_default('company'),
						...
						frappe.call({
							...
							callback: function (r_refresh) {
								if (r_refresh.message && r_refresh.message.success) {
									render_comparison_results(page, { date_wise: r_refresh.message.result }, true, false, account_info);
								}
							}
						});
					};
					show_transaction_comparison_dialog(date, account_info.account_name, r.message, refresh_fn);
				}
			}
		});
	});
		...
		frappe.call({
			...
			callback: function (r) {
				if (r.message && r.message.length > 0) {
					let rows_html = r.message.map(res => `
						<tr>
							<td>${res.date || ''}</td>
							<td>${res.account || ''}</td>
							<td>${res.type || ''}</td>
							...
							action: function (values) {
								frappe.call({
									...
									callback: function (r2) {
										if (r2.message) {
											frappe.show_alert({
												message: __('Journal Entry {0} created successfully', [r2.message]),
												indicator: 'green'
											});
											msg_dialog.hide();
											d.hide();
					...
					frappe.msgprint(__('No related journal entry found in QuickBooks report for this date and amount.'));
				}
			}
		});
	});
	...
	table_data.forEach((row, row_idx) => {
		...
		if (row_formatter) {
			...
			if (container_id === 'date-wise-table') {
				row_class = row._matched ? 'table-success text-success' : 'table-danger text-danger';
			}
		}
