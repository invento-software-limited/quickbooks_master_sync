...
function add_filter_controls(page, wrapper) {
	...
	$(wrapper)
		...
		.on("click", function () {
			...
			page.filters.list_full_response_only = page.reconcile_list_checkbox.is(":checked")
				? 1
				: 0;
...
function load_debug_files(page) {
	page.main.html(
		'<div class="text-center" style="padding: 40px;"><i class="fa fa-spinner fa-spin fa-2x"></i><br><br>Loading debug files...</div>'
	);
...
function render_files_list(page, data) {
	...
	if (data.filters_applied) {
		...
		if (active_filters.length > 0) {
			filter_status = `<small class="text-muted"><i class="fa fa-filter"></i> Filters: ${active_filters.join(
				", "
			)}</small>`;
		}
	}
	...
	let sort_icon =
		data.filters_applied && data.filters_applied.sort_order === "asc"
	...
	if (data.files.length === 0) {
		let no_results_msg =
			...
			(data.filters_applied.filename_filter ||
				data.filters_applied.data_type ||
				data.filters_applied.company_filter)
			...
			html += `
				<tr>
					...
					<td>
						${
							file.company
								? '<span class="badge badge-primary">' + file.company + "</span>"
...
function delete_file(filename, page) {
	frappe.confirm(__("Are you sure you want to delete {0}?", [filename]), function () {
		frappe.call({
			...
			if (r.message && r.message.success) {
				...
				if (log_files.length === 0) {
					frappe.msgprint({
						...
						message: __(
							"No import log files found. Log files are created when you sync data from QuickBooks."
						),
						indicator: "blue",
					});
					return;
				...
				callback: function (r) {
					if (r.message && r.message.success) {
						...
						dialog.set_df_property(
							"run_filter",
							"options",
							["All Runs"].concat(analysis.run_ids)
						);
						dialog.set_df_property(
							"event_filter",
							"options",
							["All Events"].concat(analysis.event_types)
						);
						...
						dialog.fields_dict.log_content.$wrapper.html(
							render_log_table(log_entries, {})
						);

						frappe.show_alert(
							{ message: __("Log refreshed successfully"), indicator: "green" },
							3
						);
					} else {
						frappe.msgprint({
...
function analyze_log_entries(log_entries) {
	...
	log_entries.forEach((entry) => {
		...
		if (entry.level && Object.prototype.hasOwnProperty.call(levels, entry.level)) {
			levels[entry.level]++;
		}
...
function render_log_table(log_entries, filters) {
	...
	log_entries.forEach((entry) => {
		...
		if (!Object.prototype.hasOwnProperty.call(id_to_group_index, entry_id)) {
			group_counter++;
	...
	log_entries.forEach((entry, idx) => {
		...
		if (is_first_in_group) {
			...
			group_row_count = log_entries.filter(
				(e) => (e.id || e.source || "unknown") === entry_id
			).length;
		}

		let index_display = is_first_in_group ? `${group_index}/${total_groups}` : "";

		// Add rowspan for the first row in a group
		let index_cell =
			is_first_in_group && group_row_count > 1
...
function get_level_badge(level) {
	...
	return (
		badges[level] || `<span class="badge" style="background: #999;">${level || "N/A"}</span>`
	);
}
...
function get_event_badge(event) {
	...
	return `<span class="badge" style="background: ${color}; font-size: 10px;">${
		event || "N/A"
	}</span>`;
}
...
function get_entry_details(entry) {
	...
	if (entry.error) {
		details.push(
			`<strong>Error:</strong> <span style="color: #F44336;">${frappe.utils.escape_html(
				entry.error
			)}</span>`
		);
	}
...
function export_log_to_csv(filename, log_entries) {
	let csv =
		"Timestamp,Level,Event,Run ID,Source,ID,Action,Reason,Error,Duration (ms),Progress\n";
