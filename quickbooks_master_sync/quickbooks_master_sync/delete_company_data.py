...
def delete_company_data(company_name=None, dry_run=True, delete_company=False, clear_stock=False):
    ...
    _dbg("delete_company_data:start", {
    ...
    try:
        ...
        _dbg("delete_doctype:complete", {"doctype": "Stock Ledger Entry", "count": stats["deleted"].get("Stock Ledger Entry", 0)})
        ...
        _dbg("delete_doctype:complete", {"doctype": "Stock Reconciliation", "count": stats["deleted"].get("Stock Reconciliation", 0)})
        ...
        _dbg("delete_doctype:complete", {"doctype": "Payment Ledger Entry", "count": stats["deleted"].get("Payment Ledger Entry", 0)})
        ...
        _dbg("delete_doctype:complete", {"doctype": "Journal Entry", "count": stats["deleted"].get("Journal Entry", 0)})
        ...
        _dbg("delete_doctype:complete", {"doctype": "Payment Entry", "count": stats["deleted"].get("Payment Entry", 0)})
        ...
        _dbg("delete_doctype:complete", {"doctype": "Sales Invoice", "count": stats["deleted"].get("Sales Invoice", 0)})
        ...
        _dbg("delete_doctype:complete", {"doctype": "Purchase Invoice", "count": stats["deleted"].get("Purchase Invoice", 0)})
        ...
        _dbg("delete_doctype:complete", {"doctype": "Sales Order", "count": stats["deleted"].get("Sales Order", 0)})
        ...
        _dbg("delete_doctype:complete", {"doctype": "Purchase Order", "count": stats["deleted"].get("Purchase Order", 0)})
        ...
        _dbg("delete_doctype:complete", {"doctype": "Quotation", "count": stats["deleted"].get("Quotation", 0)})
        ...
        _dbg("delete_doctype:complete", {"doctype": "Delivery Note", "count": stats["deleted"].get("Delivery Note", 0)})
        ...
        _dbg("delete_doctype:complete", {"doctype": "Purchase Receipt", "count": stats["deleted"].get("Purchase Receipt", 0)})
        ...
        _dbg("delete_doctype:complete", {"doctype": "Address", "count": address_count, "disabled": address_disabled})
        ...
        _dbg("delete_doctype:complete", {"doctype": "Contact", "count": contact_count, "disabled": contact_disabled})
        ...
        _dbg("delete_doctype:complete", {"doctype": "Quickbooks Log", "count": stats["deleted"].get("Quickbooks Log", 0)})
        ...
        if delete_company:
            ...
            _dbg("delete_doctype:complete", {"doctype": "Company", "count": stats["deleted"].get("Company", 0)})
...
def _clear_warehouse_stock(company_name, stats):
    ...
    try:
        ...
        _dbg("clear_warehouse_stock:created", {
            "stock_reconciliation": sr.name,
            "items_cleared": len(items_with_stock)
        })

    except Exception as e:
        error_msg = f"Failed to clear warehouse stock: {str(e)}"
        stats["errors"].append(error_msg)
...
def _delete_stock_ledger_entries(company_name, stats, dry_run):
    ...
    try:
        ...
        stats["errors"].append(f"{doctype}: {str(e)}")
...
def _delete_journal_entries(company_name, stats, dry_run):
    ...
    try:
                    ...
                    if doc.docstatus == 1:
                        try:
                            ...
                            if "Could not find Party" in error_msg or "Party" in error_msg:
                                _dbg(f"delete_{doctype}:cancel_skip_party_error", {
                                    "name": name,
                                    "error": error_msg,
        ...
        if error_count > 0:
            stats["warnings"].append(f"{doctype}: {error_count} entries had errors during deletion (may have missing party references)")
        _dbg(f"delete_{doctype}", {"count": count, "deleted": deleted_count, "errors": error_count, "dry_run": dry_run})
    except Exception as e:
        stats["errors"].append(f"{doctype}: {str(e)}")
            ...
            for name in all_accounts:
                try:
                    ...
                    if has_transactions > 0:
                        _dbg(f"delete_{doctype}:skip_has_transactions", {
                            "name": name,
                            "transaction_count": has_transactions
                        })
                        skipped_count += 1
                        continue

                    # Check if account has child nodes
                    has_children = frappe.db.count(doctype, {
                        "parent_account": name
                    })

                    if has_children > 0:
                        _dbg(f"delete_{doctype}:skip_has_children", {
                            "name": name,
        ...
        if not dry_run and count > 0:
            for name in addresses:
                try:
                    ...
                    is_linked_error = (
                        "linked with" in error_lower or
                        "cannot delete" in error_lower or
                        "linked to" in error_lower or
                        "cannot be deleted" in error_lower or
                        "is linked" in error_lower
                    )
                    ...
                    if is_linked_error:
                        try:
                            ...
                            disabled_count += 1
                            _dbg(f"delete_{doctype}:disabled", {
                                "name": name,
                                "reason": "Linked to transactions - disabled instead"
                            })
                        except Exception as disable_err:
                            _dbg(f"delete_{doctype}:disable_error", {
                                "name": name,
                                "error": str(disable_err)
                            })
        ...
        while True:
            try:
                ...
                for log in all_logs:
                    ...
                    if (company_lower in message or company_lower in request_data or
                        (abbr_lower and (abbr_lower in message or abbr_lower in request_data)) or
                        company_lower in method):
                ...
                error_msg = f"Error processing batch at start={start}: {str(batch_error)}"
            ...
            for i in range(0, count, delete_batch_size):
                ...
                for name in batch:
                    try:
                        ...
                        _dbg(f"delete_{doctype}:error", {"name": name, "error": str(e)})
                        stats["errors"].append(f"{doctype} ({name}): {str(e)}")
...
def _delete_debug_log_files(company_name, stats, dry_run):
    ...
    try:
        ...
        if not dry_run and count > 0:
            for filename in deleted_files:
                try:
                    ...
                    stats["errors"].append(f"Debug Log File ({filename}): {str(e)}")
        ...
        stats["errors"].append(f"Debug Log Files: {str(e)}")
