...
def sync_opening_balances(quickbooks_obj, cutoff_date=None, auto_submit=True):
    ...
    sync_ar_opening_balances(
        quickbooks_obj, qb_company, cutoff_date, stats, auto_submit, cache=cache
    )

    # 2. Sync AP by Vendor
    _dbg("sync_opening_balances:ap_start")
    frappe.logger().info("[Opening Balances] Starting AP (Accounts Payable) sync")
    sync_ap_opening_balances(
        quickbooks_obj, qb_company, cutoff_date, stats, auto_submit, cache=cache
    )
    ...
    sync_inventory_opening_balances(
        quickbooks_obj, qb_company, cutoff_date, stats, auto_submit, cache=cache
    )
    ...
    frappe.logger().info(f"[Opening Balances] Sync completed - Bank: {stats.get('bank_accounts', 0)}, Errors: {stats.get('errors', 0)}")
    ...
    if stats.get("bank_accounts", 0) > 0:
        qb_log_status(
            ...
            request_data={
                "bank_accounts": stats.get("bank_accounts", 0),
    ...
    if stats.get("bank_accounts", 0) == 0 and stats.get("errors", 0) == 0:
        qb_log_status(
            ...
            method="sync_opening_balances",
            status="Error",
            message=_("No bank accounts were processed. This might indicate accounts were not found in ERPNext or all balances were zero."),
            module="sync_opening_balances",
        )
...
def process_ar_opening_chunk(customer_list, qb_company=None, cutoff_date=None, auto_submit=False):
    ...
    try:
        ...
        # Process the chunk
        stats = sync_qb_ar_opening_chunk(
            customer_list, 
            quickbooks_obj, 
            qb_company, 
            cutoff_date, 
...
def sync_qb_ar_opening_chunk(customers, quickbooks_obj, company, cutoff_date, auto_submit=False):
    ...
    tracker = ImportTracker(len(customers), "qb_ar_opening_import.log", company=company, module_name="AR_OPENING")
    ...
    for qb_customer in customers:
        try:
            ...
            existing_je = frappe.db.get_value(
                "Journal Entry",
                {"quickbooks_journal_entry_id": unique_je_id},
                "name"
            )
            ...
            if not erp_customer:
                ...
                _dbg("sync_ar_opening_balances:customer_not_found", {
                    "qb_id": customer_id,
                    "qb_name": customer_name,
                    "company": company
                })
            ...
            _dbg("sync_ar_opening_balances:created", {
                "customer": erp_customer.name,
                "amount": balance
            })

        except Exception as e:
            _dbg("sync_ar_opening_balances:error", {
                "customer": qb_customer.get("DisplayName"),
                "error": str(e)
            })
            stats["errors"] += 1
            qb_log_exception(
...
def sync_ar_opening_balances(
    ...
    try:
        ...
        qb_log_status(
            ...
            status="Queued",
            method="sync_ar_opening_balances",
            message=_("Enqueued {0} customers in chunks of {1} for background processing.").format(total_customers, chunk_size),
            module="sync_opening_balances"
        )
...
def create_ar_opening_journal_entry(
    company, customer, customer_name, amount, posting_date, quickbooks_id, 
    auto_submit=False, cache=None
):
    """Create Journal Entry for AR opening balance"""
    ...
    if not company_default_receivable:
        company_default_receivable = frappe.db.get_value(
    ...
    receivable_currency = frappe.db.get_value("Account", receivable_account, "account_currency") or company_currency
    ...
    try:
        ...
        if not account_exists:
            ...
            if hasattr(customer_doc, "accounts"):
                ...
                _dbg("create_ar_opening_journal_entry:customer_account_linked_dynamically", {
                    "customer": customer,
                    "account": receivable_account,
                    "company": company,
        ...
        _dbg("create_ar_opening_journal_entry:account_linking_error", {
            "account": receivable_account,
            "customer": customer,
    ...
    try:
        ...
        frappe.db.rollback()
        _dbg("create_ar_opening_journal_entry:insert_error", {
            "error": str(e),
            "customer": customer,
            "amount": amount
        })
...
def process_ap_opening_chunk(vendor_list, qb_company=None, cutoff_date=None, auto_submit=False):
    ...
    try:
        ...
        # Process the chunk
        stats = sync_qb_ap_opening_chunk(
            vendor_list, 
            quickbooks_obj, 
            qb_company, 
            cutoff_date, 
...
def sync_qb_ap_opening_chunk(vendors, quickbooks_obj, company, cutoff_date, auto_submit=False):
    ...
    # Initialize generic Import Tracker
    from ..utils.import_tracker import ImportTracker
    tracker = ImportTracker(len(vendors), "qb_ap_opening_import.log", company=company, module_name="AP_OPENING")
    ...
    for qb_vendor in vendors:
        try:
            ...
            if not erp_supplier:
                ...
                _dbg("sync_ap_opening_balances:vendor_not_found", {
                    "qb_id": vendor_id,
                    "qb_name": vendor_name,
                    "company": company
                })
            ...
            stats["ap_vendors"] += 1
            tracker.log_success("QuickBooks", tracker_id, "CREATED")
            _dbg("sync_ap_opening_balances:created", {
                "supplier": erp_supplier.name,
                "amount": balance
            })

        except Exception as e:
            _dbg("sync_ap_opening_balances:error", {
                "vendor": qb_vendor.get("DisplayName"),
                "error": str(e)
            })
            stats["errors"] += 1
            qb_log_exception(
...
def sync_ap_opening_balances(
    ...
    try:
        ...
        qb_log_status(
            ...
            status="Queued",
            method="sync_ap_opening_balances",
            message=_("Enqueued {0} vendors in chunks of {1} for background processing.").format(total_vendors, chunk_size),
            module="sync_opening_balances"
        )
...
def create_ap_opening_journal_entry(
    company, supplier, supplier_name, amount, posting_date, quickbooks_id, 
    auto_submit=False, cache=None
):
    """Create Journal Entry for AP opening balance"""
    ...
    if not company_default_payable:
        company_default_payable = frappe.db.get_value(
    ...
    try:
        ...
        if not account_exists:
            ...
            if hasattr(supplier_doc, "accounts"):
                ...
                _dbg("create_ap_opening_journal_entry:supplier_account_linked_dynamically", {
                    "supplier": supplier,
                    "account": payable_account,
                    "company": company,
        ...
        # Don't fail the journal entry if account linking fails
        _dbg("create_ap_opening_journal_entry:account_linking_error", {
            "account": payable_account,
            "supplier": supplier,
            "error": str(link_error),
    ...
    try:
        ...
        frappe.db.rollback()
        _dbg("create_ap_opening_journal_entry:insert_error", {
            "error": str(e),
            "supplier": supplier,
            "amount": amount
        })
...
def sync_bank_opening_balances(quickbooks_obj, company, cutoff_date, stats, auto_submit=False, cache=None):
    ...
    try:
        ...
        bank_query = """SELECT Id, Name, CurrentBalance, AccountType FROM Account WHERE AccountType = 'Bank'"""
        ...
        frappe.logger().info(
            f"[Opening Balances] Fetched {len(bank_accounts)} bank accounts from QuickBooks"
        )
        ...
        try:
            ...
            frappe.logger().warning(f"[Opening Balances] Failed to save debug JSON: {str(e)}")
        ...
        if accounts_with_balance:
            qb_log_status(
                ...
                status="Success",
                message=_("Processing {0} bank accounts ({1} positive, {2} negative) out of {3} total bank accounts").format(
                    len(accounts_with_balance), len(positive_balance), len(negative_balance), len(bank_accounts)
                ),
                module="sync_opening_balances",
                request_data={
        ...
        # Initialize generic Import Tracker
        from ..utils.import_tracker import ImportTracker
        tracker = ImportTracker(len(bank_accounts), "qb_bank_opening_import.log", company=company, module_name="BANK_OPENING")
        ...
        for qb_account in bank_accounts:
            try:
                ...
                existing_je = frappe.db.get_value(
                    "Journal Entry",
                    {"quickbooks_journal_entry_id": unique_je_id, "company": company},
                    "name"
                )
                ...
                if not erp_account:
                    erp_account = frappe.db.get_value(
                        "Account",
                        {"quickbooks_account_id": unique_account_id, "company": company},
                        "name"
                    )
                ...
                _dbg("sync_bank_opening_balances:error", {
                    "account": qb_account.get("Name"),
                    "error": str(e)
                })
                frappe.logger().error(f"[Opening Balances] Error processing account {qb_account.get('Name')}: {str(e)}")
                stats["errors"] += 1
                qb_log_exception(
                    method="sync_bank_opening_balances",
                    err=e,
        ...
        frappe.logger().error(f"[Opening Balances] Query error in bank sync: {str(e)}")
        qb_log_exception(
            method="sync_bank_opening_balances",
            err=e,
            request_data={},
...
def create_bank_opening_journal_entry(company, bank_account, account_name, amount, posting_date, quickbooks_id, auto_submit=False):
    """Create Journal Entry for bank opening balance"""
    ...
    existing = frappe.db.get_value(
        "Journal Entry",
        {"quickbooks_journal_entry_id": qb_je_id, "company": company},
        "name"
    )

    if existing:
        _dbg("create_bank_opening_journal_entry:exists", {
            "name": existing,
            "qb_je_id": qb_je_id,
    ...
    _dbg("create_bank_opening_journal_entry:creating", {
        "company": company,
    ...
    frappe.logger().info(f"[Opening Balances] Creating JE for {account_name} (Account: {bank_account}, Amount: {amount})")
    ...
    try:
        ...
        _dbg("create_bank_opening_journal_entry:inserted", {
            "name": je.name,
        ...
        if auto_submit:
            je.submit()
            _dbg("create_bank_opening_journal_entry:success", {
                "name": je.name,
                "status": "submitted",
                "docstatus": je.docstatus
            })
            frappe.logger().info(f"[Opening Balances] ✓ JE {je.name} submitted successfully for {account_name}")
        else:
            _dbg("create_bank_opening_journal_entry:success", {
                "name": je.name,
                "status": "draft",
        ...
        frappe.db.rollback()
        _dbg("create_bank_opening_journal_entry:insert_error", {
            "error": str(e),
        ...
        frappe.logger().error(
            f"[Opening Balances] ✗ Failed to create JE for {account_name}: {type(e).__name__}: {str(e)}"
        )
        raise
...
def sync_loan_opening_balances(quickbooks_obj, company, cutoff_date, stats, auto_submit=False, cache=None):
    ...
    try:
        ...
        # Initialize generic Import Tracker
        from ..utils.import_tracker import ImportTracker
        tracker = ImportTracker(len(loan_accounts), "qb_loan_opening_import.log", company=company, module_name="LOAN_OPENING")
        ...
        for qb_account in loan_accounts:
            try:
                ...
                if not erp_account:
                    ...
                    _dbg("sync_loan_opening_balances:account_not_found", {
                        "qb_id": account_id,
                        "qb_name": account_name,
                        "company": company
                    })
                ...
                stats["loan_accounts"] += 1
                tracker.log_success("QuickBooks", tracker_id, "CREATED")
                _dbg("sync_loan_opening_balances:created", {
                    "account": erp_account,
                    "amount": balance
                })

            except Exception as e:
                _dbg("sync_loan_opening_balances:error", {
                    "account": qb_account.get("Name"),
                    "error": str(e)
                })
                stats["errors"] += 1
...
def create_loan_opening_journal_entry(company, loan_account, account_name, amount, posting_date, quickbooks_id, auto_submit=False):
    """Create Journal Entry for loan opening balance"""
    ...
    try:
        ...
        frappe.db.rollback()
        _dbg("create_loan_opening_journal_entry:insert_error", {
            "error": str(e),
...
def sync_qb_inventory_opening_chunk(inventory_items, quickbooks_obj, company, cutoff_date, auto_submit=False):
    ...
    # Initialize generic Import Tracker
    from ..utils.import_tracker import ImportTracker
    tracker = ImportTracker(len(inventory_items), "qb_inventory_opening_import.log", company=company, module_name="INVENTORY_OPENING")
    ...
    for qb_item in inventory_items:
        try:
            ...
            erp_item = frappe.db.get_value(
                "Item",
                {"quickbooks_item_id": unique_item_id},
                "name"
            )

            if not erp_item:
                _dbg("sync_inventory_opening_balances:item_not_found", {
                    "qb_id": item_id,
                    "qb_name": item_name
                })
                stats["errors"] += 1
            ...
            if not valuation_rate or valuation_rate == 0:
                try:
                    ...
                    _dbg("sync_inventory_opening_balances:get_buying_price_failed", {
                        "erp_item": erp_item,
                        "error": str(e)
                    })
            ...
            if not valuation_rate or valuation_rate == 0:
                try:
                    ...
                    valuation_rate = (
                        item_rates.get("last_purchase_rate") or
                        item_rates.get("standard_rate") or
                        item_rates.get("valuation_rate") or
                        0
                    )
                except Exception as e:
                    _dbg("sync_inventory_opening_balances:get_item_rates_failed", {
                        "erp_item": erp_item,
                        "error": str(e)
                    })
                    valuation_rate = 0
            ...
            if not valuation_rate or valuation_rate == 0:
                _dbg("sync_inventory_opening_balances:no_valuation_rate_skipping", {
                    "qb_id": item_id,
                    "item_name": item_name
                })
                ...
                qb_log_status(
                    ...
                    message=_(
                        "Item {0} ({1}) skipped because no valuation rate could be found."
                    ).format(item_name, erp_item),
                    module="sync_opening_balances",
                    exception=False,
                )
                stats["errors"] += 1
            ...
            stats["inventory_items"] += 1
            tracker.log_success("QuickBooks", tracker_id, "CREATED")
            _dbg("sync_inventory_opening_balances:created", {
                "item": erp_item,
                "qty": qty
            })

        except Exception as e:
            _dbg("sync_inventory_opening_balances:error", {
                "item": qb_item.get("Name"),
                "error": str(e)
            })
            stats["errors"] += 1
            qb_log_exception(
...
def sync_inventory_opening_balances(quickbooks_obj, company, cutoff_date, stats, auto_submit=False, cache=None):
    """
    ...
    try:
        ...
        qb_log_status(
            ...
            status="Queued",
            method="sync_inventory_opening_balances",
            message=_("Enqueued {0} inventory items in chunks of {1} for background processing.").format(total_items, chunk_size),
            module="sync_opening_balances"
        )
...
def create_opening_stock_reconciliation(company, item_code, item_name, qty, warehouse, posting_date, quickbooks_id, valuation_rate=None, auto_submit=False):
    """Create Stock Reconciliation for opening inventory"""

    # Check if already created
    qb_sr_id = f"SR-OPEN-{quickbooks_id}"
    ...
    if valuation_rate is None or valuation_rate == 0:
        ...
        try:
            ...
            _dbg("create_opening_stock_reconciliation:get_buying_price_failed", {
                "item_code": item_code,
                "error": str(e)
            })
        ...
        if not valuation_rate or valuation_rate == 0:
            try:
                ...
                _dbg("create_opening_stock_reconciliation:get_item_rates_failed", {
                    "item_code": item_code,
                    "error": str(e)
                })
                valuation_rate = 0
    ...
    if not valuation_rate or valuation_rate <= 0:
        ...
        _dbg("create_opening_stock_reconciliation:no_valuation_rate", {
            "item_code": item_code,
            "item_name": item_name,
            "qb_id": quickbooks_id,
    ...
    _dbg("create_opening_stock_reconciliation:using_valuation_rate", {
        "item_code": item_code,
        "item_name": item_name,
        "qb_id": quickbooks_id,
        "valuation_rate": valuation_rate
    ...
    if not item_row.valuation_rate or flt(item_row.valuation_rate) <= 0:
        ...
        _dbg("create_opening_stock_reconciliation:valuation_rate_not_set", {
            "item_code": item_code,
            "item_name": item_name,
            "qb_id": quickbooks_id,
            "valuation_rate": valuation_rate,
    ...
    try:
        ...
        frappe.db.rollback()
        _dbg("create_opening_stock_reconciliation:insert_error", {
            "error": str(e),
            "item_code": item_code,
            "qty": qty
        })
...
def create_equity_balancing_entry(company, cutoff_date, stats):
    ...
    try:
        ...
        balance = frappe.db.sql("""
            ...
            AND posting_date <= %s
            AND is_cancelled = 0
        """, (equity_account, company, cutoff_date), as_dict=True)
        ...
        _dbg("create_equity_balancing_entry:complete", {
...
def get_or_create_stock_adjustment_account(company):
    ...
    temp_account = frappe.db.get_value(
        ...
        {
            "company": company,
            "account_name": "Temporary Opening Stock"
        },
        "name"
    )
    ...
    try:
        ...
        parent_account = frappe.db.get_value(
            ...
            {
                "company": company,
                "account_name": ["in", ["Current Assets", "Stock Assets"]],
                "is_group": 1
            },
            "name"
        )
        ...
        _dbg("get_or_create_stock_adjustment_account:created", {
            "account": account.name,
            "type": "Temporary Opening Stock (Asset)"
        })
        return account.name
...
def sync_entry(quickbooks_obj):
    ...
    """
    _dbg("sync_entry:deprecated", {
        "message": "Use sync_opening_balances() instead"
    })
