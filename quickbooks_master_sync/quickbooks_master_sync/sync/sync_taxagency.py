...
def _ensure_tax_agency_supplier_group():
    ...
    if not frappe.db.exists("Supplier Group", "Tax Agency"):
        _dbg("ensure_tax_agency_supplier_group:creating", {
            "group_name": "Tax Agency"
        })
        ...
        try:
            ...
            _dbg("ensure_tax_agency_supplier_group:created", {
                "group_name": "Tax Agency",
                "parent": parent_group
            })
        except Exception as e:
            _dbg("ensure_tax_agency_supplier_group:error", {
                "error": str(e)
            })
...
def sync_taxagency(quickbooks_obj, qb_company=None):
    ...
    taxagency_list = query_with_pagination(
        ...
        taxagency_query,
        "TaxAgency",
        module_name="sync_taxagency",
        data_name="taxagency_list"
    )
    ...
    _dbg("sync_taxagency:start", {
        "total_taxagencies": len(taxagency_list),
        "company": qb_company
    })
...
def _sync_single_taxagency(qb_taxagency, qb_company):
    ...
    _dbg("sync_single_taxagency:processing", {
    ...
    if existing_supplier:
        ...
        if changed:
            doc.save()
            _dbg("sync_single_taxagency:updated", {
                "taxagency_id": taxagency_id,
                "supplier": doc.name,
                "changes": "Updated supplier fields"
            })
            return "updated"
        else:
            _dbg("sync_single_taxagency:skipped", {
                "taxagency_id": taxagency_id,
                "supplier": doc.name,
        ...
        _dbg("sync_single_taxagency:created", {
            "taxagency_id": taxagency_id,
            "supplier": doc.name,
            "supplier_group": supplier_group
        })
