		...
		if parent_group:
			...
			if resolved_parent and existing_parent != resolved_parent:
				try:
					...
					_dbg(
						"_ensure_item_group:updated_parent_by_name",
						{"name": group_name, "parent": resolved_parent},
					)
				except Exception as e:
					_dbg(
						"_ensure_item_group:update_parent_by_name_error",
...
def create_Item(quickbooks_obj, qb_company=None):
	...
	stats = {"total": 0, "created": 0, "updated": 0, "skipped": 0, "failed": 0, "failed_items": []}

	item_query = """SELECT * FROM Item"""
	# Use pagination helper to fetch all items
	item_list = query_with_pagination(
		quickbooks_obj, item_query, "Item", module_name="sync_products", data_name="items_list"
	)
	...
	try:
		...
		tracker = ImportTracker(
			len(items), "qb_product_import.log", module_name="PRODUCT", company=resolved_company
		)
				...
				if income_ref:
					...
					if inc_qb_id:
						...
						income_account = frappe.db.get_value(
							"Account",
							{"quickbooks_account_id": inc_qb_id, "company": resolved_company},
							"name",
						)
					...
					if not income_account and income_ref.get("name"):
						income_account = frappe.db.get_value(
							"Account",
							{"account_name": income_ref.get("name"), "company": resolved_company},
							"name",
						)
				...
				if expense_ref:
					...
					if exp_qb_id:
						expense_account = frappe.db.get_value(
							"Account",
							{"quickbooks_account_id": exp_qb_id, "company": resolved_company},
							"name",
						)
					...
					if not expense_account and expense_ref.get("name"):
						expense_account = frappe.db.get_value(
							"Account",
							{"account_name": expense_ref.get("name"), "company": resolved_company},
							"name",
						)
				...
				if asset_ref:
					...
					if asset_qb_id:
						asset_account = frappe.db.get_value(
							"Account",
							{"quickbooks_account_id": asset_qb_id, "company": resolved_company},
							"name",
						)
					...
					if not asset_account and asset_ref.get("name"):
						asset_account = frappe.db.get_value(
							"Account",
							{"account_name": asset_ref.get("name"), "company": resolved_company},
							"name",
						)
				...
				if income_account or expense_account or asset_account or stores_warehouse:
					...
					for d in doc.get("item_defaults"):
						if d.company == resolved_company:
							...
							if asset_account:
								d.custom_default_asset_account = asset_account
							if stores_warehouse:
								d.default_warehouse = stores_warehouse
							found = True
							break
					if not found:
						doc.append(
...
def create_opening_stock_entry(item_code, qty, posting_date, warehouse, company, valuation_rate=None):
		...
		if valuation_rate is None or valuation_rate == 0:
			...
			try:
				item_rates = frappe.db.get_value(
					"Item", item_code, ["last_purchase_rate", "standard_rate"], as_dict=True
				)
		...
		if not frappe.db.exists("Item", {"item_code": item_code}):
			...
			if " - " in item_code:
				...
				if frappe.db.exists("Item", {"item_code": base_code}):
					...
					if base_code_trimmed != base_code and frappe.db.exists(
						"Item", {"item_code": base_code_trimmed}
					):
