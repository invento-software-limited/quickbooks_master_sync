...
def sync_suppliers(quickbooks_obj):
	...
	get_qb_supplier = query_with_pagination(
		quickbooks_obj, supplier_query, "Vendor", module_name="sync_suppliers", data_name="suppliers_list"
	)
...
def sync_qb_suppliers(get_qb_supplier, quickbooks_supplier_list, stats=None, qb_company=None):
	if stats is None:
		stats = {"created": 0, "updated": 0, "skipped": 0, "errors": 0, "failed_suppliers": []}
	...
	tracker = ImportTracker(
		len(get_qb_supplier), "qb_supplier_import.log", company=qb_company, module_name="SUPPLIER"
	)

	_dbg("sync_qb_suppliers:start", {"total_suppliers": len(get_qb_supplier), "qb_company": qb_company})
	...
	for idx, qb_supplier in enumerate(get_qb_supplier):
		...
		supplier_display_name = (
			qb_supplier.get("DisplayName")
			or qb_supplier.get("CompanyName")
			or qb_supplier.get("name")
			or "Unknown"
		)
		...
		if not existing_name:
			...
			try:
				result = create_Supplier(qb_supplier, quickbooks_supplier_list, qb_company)
				...
				stats["failed_suppliers"].append(
					{"qb_id": qb_id, "name": supplier_name, "error": str(e), "action": "create"}
				)
				_dbg(
				...
				qb_log_error(
					...
					message=_("Failed to create supplier '{0}' (QuickBooks ID: {1}). " "Error: {2}").format(
						supplier_name, qb_id, str(e)
					),
					module="sync_suppliers",
			...
			try:
				...
				stats["errors"] += 1
				supplier_name = (
					qb_supplier.get("DisplayName") or qb_supplier.get("CompanyName") or existing_name
				)
				tracker.log_error("QuickBooks", tracker_id, str(e))
				stats["failed_suppliers"].append(
					{"qb_id": qb_id, "name": supplier_name, "error": str(e), "action": "update"}
				)
				_dbg(
				...
				qb_log_error(
					...
					status="Error",
					method="sync_qb_suppliers",
					message=_("Failed to update supplier '{0}' (QuickBooks ID: {1}). " "Error: {2}").format(
						supplier_name, qb_id, str(e)
					),
					module="sync_suppliers",
					request_data=qb_supplier,
...
def create_Supplier(qb_supplier, quickbooks_supplier_list, qb_company=None):
	...
	try:
		...
		qb_supplier_id = (
			str(qb_supplier.get("Id")) if qb_supplier.get("Id") else str(qb_supplier.get("value"))
		)
		...
		if existing_supplier:
			...
			existing_qb_id = frappe.db.get_value("Supplier", existing_supplier, "quickbooks_supp_id")
		...
		if qb_supplier.get("PrimaryEmailAddr"):
			if isinstance(qb_supplier.get("PrimaryEmailAddr"), dict):
				qb_supplier["PrimaryEmailAddr"].get("Address", "")
			else:
				str(qb_supplier.get("PrimaryEmailAddr"))

		# Extract phone numbers
		fax_number = None
		if qb_supplier.get("PrimaryPhone"):
			if isinstance(qb_supplier.get("PrimaryPhone"), dict):
				qb_supplier["PrimaryPhone"].get("FreeFormNumber", "")
			else:
				str(qb_supplier.get("PrimaryPhone"))
		if qb_supplier.get("Mobile"):
			if isinstance(qb_supplier.get("Mobile"), dict):
				qb_supplier["Mobile"].get("FreeFormNumber", "")
			else:
				str(qb_supplier.get("Mobile"))
		...
		supplier_payload = {
			...
			"default_currency": (
				qb_supplier["CurrencyRef"].get("value", "") if qb_supplier.get("CurrencyRef") else ""
			),
			...
			try:
				...
				if meta.has_field("disabled"):
					...
					if is_deleted:
						...
						_dbg(
							"create_Supplier:deleted_supplier_not_disabled",
							{
								"supplier_name": supplier_name,
		...
		try:
			...
			if duplicate_name:
				...
				existing_qb_id = frappe.db.get_value("Supplier", duplicate_name, "quickbooks_supp_id")
			...
			existing_by_name = frappe.db.get_value("Supplier", {"supplier_name": supplier_name}, "name")
			if existing_by_name:
				_dbg(
...
def update_Supplier(qb_supplier, existing_name, quickbooks_supplier_list, qb_company=None):
				...
				try:
					...
					if meta.has_field("supplier_print_name"):
						if getattr(supplier, "supplier_print_name", None) != print_name:
							old_print_name = getattr(supplier, "supplier_print_name", None)
					...
					if erpnext_account:
						...
						if hasattr(supplier, "accounts"):
							for acc_row in supplier.get("accounts", []):
								if (
									acc_row.get("account") == erpnext_account
									and acc_row.get("company") == resolved_company
								):
									account_exists = True
									break
...
def create_supplier_address(supplier, address):
	address_title, resolved_address_type = get_address_title_and_type(supplier.supplier_name)
	try:
		# Build address lines
		...
		address_payload = {
			"doctype": "Address",
			"quickbooks_address_id": (str(address.get("Id")) if address.get("Id") else None),
...
def create_supplier_contact(supplier, qb_supplier):
	...
	try:
		...
		if not first_name:
			_dbg(
				"create_supplier_contact:no_first_name",
				{"supplier": supplier.name, "qb_id": qb_supplier.get("Id")},
			)
			return
		...
		existing_contact = frappe.db.get_value(
			"Dynamic Link",
			{"link_doctype": "Supplier", "link_name": supplier.name, "parenttype": "Contact"},
			"parent",
		)
		...
		if existing_contact:
			...
			_dbg(
				"create_supplier_contact:updating_existing",
				{"supplier": supplier.name, "contact": existing_contact},
			)
			...
			_dbg("create_supplier_contact:updated", {"supplier": supplier.name, "contact": contact.name})
			return
		...
		_dbg(
			"create_supplier_contact:success",
			{
				"supplier": supplier.name,
		...
		_dbg("create_supplier_contact:error", {"supplier": supplier.name, "error": str(e)})
			...
			for response_obj in response_from_quickbooks.successes:
				if response_obj:
					...
					supplier_company = frappe.db.get_value(
						"Supplier", {"supplier_name": response_obj.DisplayName}, "custom_company"
					)
					if not supplier_company:
						supplier_company = frappe.db.get_value(
							"Supplier", {"supplier_name": response_obj.DisplayName}, "company"
						)
					if supplier_company:
...
def erp_supplier_data():
	erp_supplier = frappe.db.sql(
		("select `supplier_name` from `tabSupplier` " "WHERE  quickbooks_supp_id IS NULL"),
		as_dict=1,
	)
	return erp_supplier
...
def get_all_suppliers_json(source="quickbooks", quickbooks_obj=None, include_addresses=True):
	"""
	...
	if source == "erpnext":
		...
		return get_quickbooks_suppliers_json(quickbooks_obj, include_addresses=include_addresses)
	else:
...
def get_erpnext_suppliers_json(include_addresses=True):
	...
	try:
		...
		suppliers = frappe.get_all("Supplier", fields=fields_to_query, order_by="supplier_name")
				...
				for field in meta.fields:
					if field.fieldname.startswith("custom_"):
						if hasattr(doc, field.fieldname):
							supplier_dict[field.fieldname] = getattr(doc, field.fieldname, None)
			except Exception:
				pass
...
def get_suppliers_json_api(source: str = "erpnext", include_addresses: bool | str = True):
	...
	try:
		if source == "quickbooks":
			...
			json_data = get_quickbooks_suppliers_json(quickbooks_obj, include_addresses=include_addresses)
		else:
		...
		frappe.log_error(f"Error getting suppliers JSON: {e!s}", "get_suppliers_json_api")
		return {
			"success": False,
			"error": str(e),
			"source": source,
