...
def _resolve_currency(ci):
    ...
    country_currency_map = {
        ...
        "AI": "XCD", "AG": "XCD", "AR": "ARS", "AM": "AMD", "AW": "AWG", "AU": "AUD",
        "AT": "EUR", "AZ": "AZN", "BS": "BSD", "BH": "BHD", "BD": "BDT", "BB": "BBD",
        "BY": "BYN", "BE": "EUR", "BZ": "BZD", "BJ": "XOF", "BM": "BMD", "BT": "BTN",
        "BO": "BOB", "BA": "BAM", "BW": "BWP", "BR": "BRL", "BN": "BND", "BG": "BGN",
        "BF": "XOF", "BI": "BIF", "KH": "KHR", "CM": "XAF", "CA": "CAD", "CV": "CVE",
        "KY": "KYD", "CF": "XAF", "TD": "XAF", "CL": "CLP", "CN": "CNY", "CO": "COP",
        "KM": "KMF", "CG": "XAF", "CD": "CDF", "CR": "CRC", "HR": "HRK", "CU": "CUP",
        "CY": "EUR", "CZ": "CZK", "DK": "DKK", "DJ": "DJF", "DM": "XCD", "DO": "DOP",
        "EC": "USD", "EG": "EGP", "SV": "USD", "GQ": "XAF", "ER": "ERN", "EE": "EUR",
        "ET": "ETB", "FK": "FKP", "FJ": "FJD", "FI": "EUR", "FR": "EUR", "GA": "XAF",
        "GM": "GMD", "GE": "GEL", "DE": "EUR", "GH": "GHS", "GI": "GIP", "GR": "EUR",
        "GD": "XCD", "GT": "GTQ", "GN": "GNF", "GW": "XOF", "GY": "GYD", "HT": "HTG",
        "HN": "HNL", "HK": "HKD", "HU": "HUF", "IS": "ISK", "IN": "INR", "ID": "IDR",
        "IR": "IRR", "IQ": "IQD", "IE": "EUR", "IL": "ILS", "IT": "EUR", "JM": "JMD",
        "JP": "JPY", "JO": "JOD", "KZ": "KZT", "KE": "KES", "KI": "AUD", "KP": "KPW",
        "KR": "KRW", "KW": "KWD", "KG": "KGS", "LA": "LAK", "LV": "EUR", "LB": "LBP",
        "LS": "LSL", "LR": "LRD", "LY": "LYD", "LI": "CHF", "LT": "EUR", "LU": "EUR",
        "MO": "MOP", "MK": "MKD", "MG": "MGA", "MW": "MWK", "MY": "MYR", "MV": "MVR",
        "ML": "XOF", "MT": "EUR", "MH": "USD", "MR": "MRO", "MU": "MUR", "MX": "MXN",
        ...
        "SM": "EUR", "ST": "STD", "SA": "SAR", "SN": "XOF", "RS": "RSD", "SC": "SCR",
        "SL": "SLL", "SG": "SGD", "SK": "EUR", "SI": "EUR", "SB": "SBD", "SO": "SOS",
        "ZA": "ZAR", "ES": "EUR", "LK": "LKR", "SD": "SDG", "SR": "SRD", "SZ": "SZL",
        "SE": "SEK", "CH": "CHF", "SY": "SYP", "TW": "TWD", "TJ": "TJS", "TZ": "TZS",
        "TH": "THB", "TG": "XOF", "TO": "TOP", "TT": "TTD", "TN": "TND", "TR": "TRY",
        "TM": "TMT", "TV": "AUD", "UG": "UGX", "UA": "UAH", "AE": "AED", "GB": "GBP",
        "UK": "GBP", "US": "USD", "UY": "UYU", "UZ": "UZS", "VU": "VUV", "VE": "VEF",
    ...
    try:
        ...
        if fieldtype == "Link":
            if options == "Account":
                ...
                account = frappe.db.get_value(
                    "Account", {"is_group": 0}, "name", order_by="name"
                )
                if not account:
                    # If no leaf accounts, try group accounts as last resort
                    account = frappe.db.get_value(
                        "Account", {}, "name", order_by="name"
                    )
                return account
...
def _resolve_default_accounts(doc):
    ...
    if not doc.default_inventory_account:
        inventory_account = frappe.db.get_value("Account", 
    ...
    if not doc.stock_adjustment_account:
        ...
        adj_account = frappe.db.get_value("Account", 
    ...
    if not doc.cost_center:
        ...
        if not cost_center:
            ...
            cost_center = frappe.db.get_value("Cost Center", {"company": doc.name, "is_group": 1, "parent_cost_center": ["in", ["", None]]}, "name")
    ...
    try:
        ...
        if address_id and not addr.get("Line1"):
            try:
                ...
                _dbg("sync_company:address_fetch_failed", {
        ...
        try:
            ...
            qb_log_error(
                ...
                status="Error",
                method="sync_quickbooks_company_with_client",
                message=_("Tax codes/classes/agencies/terms could not be auto-synced: {0}\\n\\nYou can manually sync these items later.").format(str(tax_error)),
                module="sync_company",
            )
...
def _post_company_sync_setup(company_doc):
    ...
    try:
        ...
        warehouse = frappe.db.get_value("Warehouse", 
            {"company": company_doc.name, "warehouse_name": ["like", "%Stores%"]}, 
            "name"
        )
...
def ensure_all_defaults(company_doc):
    ...
    if not frappe.db.exists("Cost Center", default_cc_name):
         ...
         if frappe.db.exists("Cost Center", {"company": company_doc.name, "cost_center_name": "Main"}):
             default_cc_name = frappe.db.get_value("Cost Center", {"company": company_doc.name, "cost_center_name": "Main"}, "name")
    
    # If still not found, search for ANY group-0 cost center
    if not frappe.db.exists("Cost Center", default_cc_name):
        default_cc_name = frappe.db.get_value("Cost Center", {"company": company_doc.name, "is_group": 0}, "name")
    ...
    defaults_config = [
        ...
        ("stock_received_but_not_billed", "Stock Received But Not Billed", "Stock Received But Not Billed", "Liability"),
        ("stock_adjustment_account", "Stock Adjustment", "Stock Adjustment", "Expense"),
        ("expenses_included_in_valuation", "Expenses Included In Valuation", "Expenses Included In Valuation", "Expense"),
        ...
        ("exchange_gain_loss_account", "Exchange Gain/Loss", "Exchange Gain or Loss", "Income"), # Or Expense? usually under income or expense, let's try finding existing.
        ("unrealized_exchange_gain_loss_account", "Unrealized Exchange Gain/Loss", "Unrealized Exchange Gain or Loss", "Income"),
    ...
    for field, name, account_type, root_type in defaults_config:
        ...
        existing_account = frappe.db.get_value("Account", 
            {"company": company_doc.name, "account_type": account_type, "is_group": 0}, 
            "name"
        )
        ...
        parent_account = frappe.db.get_value("Account", 
            {"company": company_doc.name, "root_type": root_type, "is_group": 1, "parent_account": ["is", "not set"]}, 
            "name"
        )
        ...
        if not parent_account:
             ...
             # Rough mapping
             search_term = ""
             if root_type == "Liability": search_term = "Liabilities"
             elif root_type == "Asset": search_term = "Assets"
             elif root_type == "Expense": search_term = "Expenses"
             elif root_type == "Income": search_term = "Income"
             
             if search_term:
                parent_account = frappe.db.get_value("Account",
                    {"company": company_doc.name, "account_name": ["like", f"%{search_term}%"], "is_group": 1},
                    "name"
                )
