...
def _get_quickbooks_company(
    quickbooks_obj=None, qb_company=None, use_cache=True, module_name=None
):
    """
    ...
    if quickbooks_obj and hasattr(quickbooks_obj, "company_id"):
        try:
            ...
            if _qb_company_cache is None or not use_cache:
                ...
                company_info = quickbooks_obj.get_single_object(
                    "CompanyInfo", quickbooks_obj.company_id
                )
            ...
            if qb_company_name:
                ...
                erpnext_company = frappe.db.get_value(
                    "Company", {"company_name": qb_company_name}, "name"
                )
                if erpnext_company:
                    if module_name:
                        _dbg(
...
def ensure_fiscal_year_for_date(company, posting_date, module_name=None):
    ...
    try:
        ...
        if not existing:
            ...
            existing = fy.name
            if module_name:
                _dbg(module_name, "ensure_fiscal_year:created", {
        ...
        try:
            ...
            if module_name:
                _dbg(module_name, "ensure_fiscal_year:disable_error", {
                    "fiscal_year": existing,
                    "error": str(e),
                })
        ...
        try:
            ...
            if hasattr(fy_doc, "companies"):
                ...
                if not already_linked and company:
                    ...
                    frappe.db.commit()
                    if module_name:
                        _dbg(module_name, "ensure_fiscal_year:linked_company", {
                            "fiscal_year": existing,
                            "company": company,
                        })
        except Exception as e:
            if module_name:
                _dbg(module_name, "ensure_fiscal_year:link_error", {
                    "fiscal_year": existing,
                    "company": company,
                    "error": str(e),
                })
        ...
        if module_name:
            _dbg(module_name, "ensure_fiscal_year:ok", {
                "company": company,
        ...
        if module_name:
            _dbg(module_name, "ensure_fiscal_year:error", {
                "company": company,
                "posting_date": str(posting_date),
                "error": str(e),
    ...
    try:
        ...
        if full_response is not None:
            ...
            full_response_path = os.path.join(
                private_files_path, full_response_filename
            )
...
def query_with_pagination(quickbooks_obj, base_query, object_name, module_name=None, max_results=100, data_name=None, use_cache=False):
    """
    ...
    while True:
        ...
        try:
            ...
            if module_name:
                _dbg(module_name, "query_with_pagination:page_response", {
                    "page": page_num,
                    "start_position": start_position,
            ...
            if isinstance(response, dict):
                ...
                if fault:
                    error_msg = str(fault)
                    if module_name:
                        _dbg(module_name, "query_with_pagination:fault_detected", {
                            "page": page_num,
                            "fault": error_msg
                        })
    ...
    if module_name:
        _dbg(module_name, "query_with_pagination:complete", {
    ...
    try:
        ...
        if all_pages_responses:
            ...
            if module_name:
                _dbg(module_name, "query_with_pagination:pagination_saved", {
        ...
        if module_name:
            _dbg(module_name, "query_with_pagination:cache_save_error", {
                "error": str(e),
                "data_name": data_name
            })
...
def get_account_from_quickbooks_ref(qb_account_ref, company=None, account_type=None, root_type=None, fallback_account=None, module_name=None, strict_qb_id_only=False, cache=None):
    """
    ...
    if not qb_account_ref:
        if module_name:
            _dbg(module_name, "get_account_from_qb_ref:no_ref", {
    ...
    if not company:
        if module_name:
            _dbg(module_name, "get_account_from_qb_ref:no_company", {
                "qb_ref": qb_account_ref,
                "fallback": fallback_account
            })
        return fallback_account
    ...
    if qb_account_id:
            ...
            if cached_account:
                ...
                if account_doc and account_doc.get("is_group") != 1:
                    ...
                    if account_doc:
                        ...
                        if module_name:
                            _dbg(module_name, "get_account_from_qb_ref:found_in_cache", {
                ...
                if account_type or root_type:
                    ...
                    if account_data:
                        ...
                        if account_data:
                            account = cached_account
                            if module_name:
                                _dbg(module_name, "get_account_from_qb_ref:found_in_module_cache", {
                                    "qb_account_id": qb_account_id,
                                    "unique_qb_account_id": unique_qb_account_id,
                                    "erpnext_account": account,
        ...
        if module_name:
            _dbg(module_name, "get_account_from_qb_ref:searching", {
                "qb_account_id": qb_account_id,
                "unique_qb_account_id": unique_qb_account_id,
                "company": company,
                "filters": filters,
        ...
        if not account and account_type and not strict_qb_id_only:
            ...
            if account and module_name:
                _dbg(module_name, "get_account_from_qb_ref:found_by_id_fallback", {
                    "qb_account_id": qb_account_id,
                    "unique_qb_account_id": unique_qb_account_id,
                    "erpnext_account": account,
        ...
        if account:
            ...
            if module_name:
                _dbg(module_name, "get_account_from_qb_ref:not_found_by_id", {
                    "qb_account_id": qb_account_id,
                    "unique_qb_account_id": unique_qb_account_id,
                    "company": company,
                    "filters": filters,
    ...
    if strict_qb_id_only:
                ...
                if clean_name:
                    ...
                    if account:
                        if module_name:
                            ...
                            _dbg(module_name, "get_account_from_qb_ref:matched_deleted_by_name", {
                                "qb_account_id": qb_account_id,
                                "unique_qb_account_id": unique_qb_account_id,
                                "company": company,
        ...
        if module_name:
            ...
            _dbg(module_name, "get_account_from_qb_ref:strict_mode_not_found", {
                "qb_account_id": qb_account_id,
                "unique_qb_account_id": unique_qb_account_id,
                "company": company,
                "qb_account_name": qb_account_name,
    ...
    if not account and qb_account_name:
        ...
        if account:
            if module_name:
                _dbg(module_name, "get_account_from_qb_ref:found_by_name", {
                    "qb_account_name": qb_account_name,
                    "erpnext_account": account,
                    "qb_account_id": qb_account_id,
        ...
        if account:
            if module_name:
                _dbg(module_name, "get_account_from_qb_ref:found_by_partial_name", {
                    "qb_account_name": qb_account_name,
                    "erpnext_account": account,
    ...
    if not account and qb_account_name and ":" in qb_account_name:
        ...
        if leaf_name:
            ...
            if account:
                if module_name:
                    _dbg(module_name, "get_account_from_qb_ref:found_by_hierarchical_leaf", {
                        "qb_account_name": qb_account_name,
                        "leaf_name": leaf_name,
                        "erpnext_account": account,
    ...
    if module_name:
        _dbg(module_name, "get_account_from_qb_ref:not_found", {
            "qb_account_id": qb_account_id,
            "qb_account_name": qb_account_name,
            "account_type": account_type,
            "root_type": root_type,
...
def ensure_account_from_quickbooks_ref(
    ...
    def _enable_if_disabled(account_name, reason=None):
        ...
        try:
            ...
            if disabled:
                ...
                if module_name:
                    _dbg(module_name, "ensure_account_from_qb_ref:enabled_disabled_account", {
    ...
    if account:
        ...
        if is_group:
            ...
            if leaf:
                if module_name:
                    _dbg(module_name, "ensure_account_from_qb_ref:resolved_group_to_leaf", {
                ...
                if module_name:
                    _dbg(module_name, "ensure_account_from_qb_ref:group_resolution_failed", {
                        "group_account": account,
    ...
    try:
        ...
        if account and module_name:
            _dbg(module_name, "ensure_account_from_qb_ref:created_and_mapped", {
                "qb_account_id": qb_account_id,
                "qb_account_name": qb_account_name,
                "company": company,
                "erpnext_account": account,
        ...
        if module_name:
            _dbg(module_name, "ensure_account_from_qb_ref:error", {
                "qb_account_id": qb_account_id,
                "qb_account_name": qb_account_name,
                "company": company,
                "error": str(e),
        ...
        qb_log_exception(
            ...
            err=e,
            request_data={"qb_account_id": qb_account_id, "qb_account_name": qb_account_name, "company": company},
            module=module_name or "sync_utils",
        )
        return fallback_account
...
def get_leaf_account_from_group(account_name, company=None, account_type=None, module_name=None, cache=None):
    ...
    if not frappe.db.exists("Account", account_name):
        if module_name:
            _dbg(module_name, "get_leaf_account_from_group:not_found", {
                "account": account_name,
                "warning": "Account does not exist in ERPNext"
            })
        return None
    ...
    if cache:
        ...
        if acc_metadata:
            if not company: company = acc_metadata.get("company")
        ...
        if is_group is None:
            ...
            if acc_doc:
                if not company: company = acc_doc.get("company")
    ...
    if child_account:
        if module_name:
            _dbg(module_name, "get_leaf_account_from_group:using_child", {
    ...
    if account_type:
        ...
        if child_account:
            if module_name:
                _dbg(module_name, "get_leaf_account_from_group:using_child_no_type_filter", {
                    "group_account": account_name,
                    "child_account": child_account,
    ...
    if module_name:
        _dbg(module_name, "get_leaf_account_from_group:no_child_found", {
            "group_account": account_name,
            "company": company,
            "account_type": account_type,
...
def validate_account_for_transaction(account_name, company=None, account_type=None, module_name=None, cache=None):
    """
    ...
    if not valid_account and module_name:
        _dbg(module_name, "validate_account_for_transaction:invalid", {
...
def ensure_party_enabled(party_type, party_name, module_name=None):
    ...
    try:
        ...
        if disabled:
            ...
            frappe.db.commit()
            _dbg(module_name or "sync_utils", "ensure_party_enabled:enabled", {
        ...
        _dbg(module_name or "sync_utils", "ensure_party_enabled:error", {
            "party_type": party_type,
            "party_name": party_name,
            "error": str(e)
        })
...
def ensure_currency_exchange(from_currency, to_currency, exchange_date, exchange_rate, company=None, module_name=None):
    """
    ...
    try:
        ...
        if not from_currency or not to_currency:
            if module_name:
                _dbg(module_name, "ensure_currency_exchange:invalid_currencies", {
        ...
        if from_currency == to_currency:
            if module_name:
                _dbg(module_name, "ensure_currency_exchange:same_currency", {
                    "currency": from_currency,
                    "note": "No exchange rate needed for same currency"
                })
            return None
        ...
        try:
            ...
            if exchange_rate <= 0:
                if module_name:
                    _dbg(module_name, "ensure_currency_exchange:invalid_rate", {
            ...
            if module_name:
                _dbg(module_name, "ensure_currency_exchange:rate_conversion_error", {
                    "exchange_rate": exchange_rate,
                    "error": "Could not convert exchange rate to float"
                })
            return None
        ...
        if existing:
            if module_name:
                _dbg(module_name, "ensure_currency_exchange:exists", {
                    "from_currency": from_currency,
                    "to_currency": to_currency,
        ...
        if module_name:
            _dbg(module_name, "ensure_currency_exchange:created", {
                "from_currency": from_currency,
                "to_currency": to_currency,
                "date": str(exchange_date),
                "exchange_rate": exchange_rate,
        ...
        if module_name:
            _dbg(module_name, "ensure_currency_exchange:error", {
                "from_currency": from_currency,
                "to_currency": to_currency,
        ...
        frappe.log_error(
            f"Error ensuring Currency Exchange: {str(e)}",
            "ensure_currency_exchange"
        )
        return None
...
class SyncCache:
    ...
    def prefetch_accounts_by_qb_ids(self, qb_acc_ids):
        ...
        found = frappe.get_all(
            ...
            fields=["name", "quickbooks_account_id", "account_currency", "account_type", "account_name", "is_group", "parent_account", "company"]
        )
        ...
        for acc in found:
            ...
            is_stock = (
                acc.account_type in ["Stock", "Stock Adjustment"] or
                "inventory" in acc_name_lower or
                "inventory" in disp_name_lower or
                ("stock" in acc_name_lower and "stock adjustment" not in acc_name_lower) or
                ("stock" in disp_name_lower and "stock adjustment" not in disp_name_lower)
            )
    ...
    def get_fiscal_year_cached(self, posting_date):
        ...
        if not posting_date: return None
    ...
    def get_currency_exchange_cached(self, from_curr, to_curr, date, rate):
        ...
        if not from_curr or not to_curr or from_curr == to_curr: return None
        date_str = str(date)
    ...
    def get_fallback_account(self, account_type):
        """Pre-fetch or find fallback accounts (optimized)"""
        if account_type in self.fallbacks:
            return self.fallbacks[account_type]
            
        fallback = None
        if account_type == "Receivable":
            fallback = frappe.db.get_value("Account", {"company": self.company, "account_type": "Current Asset", "is_group": 0, "account_name": ("like", "%Sundry%")}, "name")
            if not fallback:
                 fallback = frappe.db.get_value("Account", {"company": self.company, "account_type": "Current Asset", "is_group": 0}, "name")
        elif account_type == "Payable":
            fallback = frappe.db.get_value("Account", {"company": self.company, "account_type": "Current Liability", "is_group": 0, "account_name": ("like", "%Sundry%")}, "name")
            if not fallback:
                 fallback = frappe.db.get_value("Account", {"company": self.company, "account_type": "Current Liability", "is_group": 0}, "name")
        elif account_type == "Stock":
            # Search for a current asset that is NOT stock
            assets = frappe.db.get_all("Account", filters={"company": self.company, "account_type": "Current Asset", "is_group": 0}, fields=["name", "account_name"])
            for asset in assets:
                n_lower, d_lower = asset.name.lower(), asset.account_name.lower()
                if "inventory" not in n_lower and "inventory" not in d_lower and "stock" not in n_lower and "stock" not in d_lower:
                    fallback = asset.name
                    break
            if not fallback:
                fallback = frappe.db.get_value("Account", {"company": self.company, "account_type": "Expense Account", "is_group": 0}, "name")

        self.fallbacks[account_type] = fallback
        return fallback
    ...
    def get_account_meta(self, account_name):
        ...
        if account_name not in self.account_metadata:
            ...
            acc = frappe.db.get_value("Account", account_name, ["name", "account_name", "account_type", "is_group"], as_dict=True)
            if acc:
                n_lower, d_lower = str(acc.name).lower(), str(acc.account_name).lower()
                is_stock = (
                    acc.account_type in ["Stock", "Stock Adjustment"] or
                    "inventory" in n_lower or "inventory" in d_lower or
                    ("stock" in n_lower and "stock adjustment" not in n_lower) or
                    ("stock" in d_lower and "stock adjustment" not in d_lower)
                )
    ...
    def get_party_name_cached(self, party_type, qb_id):
        ...
        if not party_type or not qb_id: return None
        ...
        if party_type == "Customer":
            ...
            erp_name = frappe.db.get_value("Supplier", {"quickbooks_supp_id": unique_id}, "name")
             ...
             erp_name = frappe.db.get_value("Employee", {"quickbooks_emp_id": unique_id, "company": self.company}, "name")
    ...
    def prefetch_parties(self, party_type, qb_ids):
        ...
        if not party_type or not qb_ids: return
        ...
        id_field = "quickbooks_cust_id" if party_type == "Customer" else "quickbooks_supp_id" if party_type == "Supplier" else "quickbooks_emp_id"
        
        found = frappe.get_all(
            party_type,
            filters={id_field: ["in", missing_ids]},
            fields=["name", id_field, "default_currency"]
        )
    ...
    def prefetch_gl_currencies(self, parties):
        ...
        for p_type, p_names in by_type.items():
            ...
            """, (p_type, p_names, self.company), as_dict=True)
            
            # Group by party
            temp = {}
            for row in data:
                p_name = row.party
                if p_name not in temp: temp[p_name] = []
    ...
    def prefetch_party_control_accounts(self, parties):
        ...
        for p_type, p_name in missing:
            acc = frappe.db.get_value("Party Account", {"parenttype": p_type, "parent": p_name, "company": self.company}, "account")
    ...
    def prefetch_for_invoice_sync(self):
        ...
        self.non_taxable_template = frappe.db.get_value("Sales Taxes and Charges Template", {"company": self.company}, "name")
    ...
    def load_data(self):
        ...
        self.company_defaults = frappe.db.get_value(
            "Company", self.company, 
            ["default_receivable_account", "default_payable_account", "default_income_account", "default_expense_account", "default_currency"], 
            as_dict=True
        ) or {}
    ...
    def get_invoice(self, qb_invoice_id):
        ...
        if qb_invoice_id not in self.invoices:
            inv = frappe.db.get_value(
                ...
                ["name", "quickbooks_invoce_id", "docstatus", "outstanding_amount", "currency", "customer", "debit_to"],
                as_dict=True
            )
    ...
    def prefetch_invoices(self, qb_invoice_ids):
        ...
        found = frappe.get_all(
            ...
            fields=["name", "quickbooks_invoce_id", "docstatus", "outstanding_amount", "currency", "customer", "debit_to"]
        )
    ...
    def get_journal_entry(self, qb_je_id):
        ...
        if qb_je_id not in self.journal_entries:
            name = frappe.db.get_value(
                "Journal Entry", 
                {"quickbooks_journal_entry_id": qb_je_id}, 
                "name"
            )
    ...
    def get_account_by_qb_id(self, qb_id):
        ...
        if qb_id not in self.accounts_by_qb_id:
            name = frappe.db.get_value(
                "Account", 
                {"quickbooks_account_id": qb_id, "company": self.company}, 
                "name"
            )
    ...
    def get_account_doc(self, name):
        ...
        if name not in self.accounts_by_name:
            acc = frappe.db.get_value(
                ...
                ["name", "quickbooks_account_id", "account_currency", "account_type", "account_name", "is_group", "parent_account", "company"],
                as_dict=True
            )
    ...
    def get_supplier(self, qb_supp_id):
        ...
        if qb_supp_id not in self.suppliers:
            supp = frappe.db.get_value("Supplier", {"quickbooks_supp_id": qb_supp_id}, ["name", "default_currency"], as_dict=True)
            if supp:
    ...
    def get_non_taxable_template(self):
        """Lazy load non-taxable item tax template"""
        if self.non_taxable_template is None:
            from .sync_utils import get_or_create_non_taxable_template
...
def get_cost_center_from_quickbooks_ref(qb_class_ref, company=None, module_name=None, cache=None):
    ...
    if module_name and cost_center:
        _dbg(module_name, "get_cost_center:found", {
            "class_id": class_id,
...
def get_payment_terms_template_from_quickbooks_ref(qb_term_ref, company=None, module_name=None, cache=None):
    ...
    if module_name and template:
        _dbg(module_name, "get_payment_terms_template:found", {
            "term_id": term_id,
            "template": template
        })
    ...
    if must_be_whole and qty != int(qty):
        ...
        try:
            ...
            if module_name:
                ...
                _dbg(module_name, "adjust_qty_and_rate_for_uom:uom_fixed", {
                    "item_code": item_code,
                    "uom": uom,
                    "qty": qty,
                    "rate": rate,
            ...
            if module_name:
                from quickbooks_master_sync.quickbooks_master_sync.utils.logging import _dbg
                _dbg(module_name, "adjust_qty_and_rate_for_uom:uom_fix_failed", {
                    "item_code": item_code,
                    "uom": uom,
                    "error": str(e)
                })
