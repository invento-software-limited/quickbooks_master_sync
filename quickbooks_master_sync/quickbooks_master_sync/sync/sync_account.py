...
def get_erpnext_root_and_account_type(qb_account):
    ...
    # --- LIABILITIES ---
        ...
        if qb_subtype in ["GlobalTaxPayable", "PayrollTaxPayable", "SalesTaxPayable"] or (qb_subtype and "Tax" in qb_subtype): 
            account_type = "Tax"
...
def _get_default_parent_account(company, root_type):
    ...
    if not parent_account:
        ...
        _dbg("get_default_parent:unknown_root_type", {
            "root_type": root_type,
            "fallback": parent_account
        })
    ...
    if not frappe.db.get_value("Account", {"name": parent_account}, "name"):
        ...
        if root_account:
            ...
            _dbg("get_default_parent:using_root_account", {
                "root_type": root_type,
            ...
            if fallback_parent:
                ...
                _dbg("get_default_parent:using_fallback", {
                    "root_type": root_type,
                    "parent_account": parent_account
                })
            else:
...
def _ensure_root_cost_centers(company):
    ...
    root_name = frappe.db.get_value("Cost Center", 
...
def _map_quickbooks_account_type_to_erpnext(qb_account_type):
    ...
    valid_erpnext_types = [
        ...
        "Bank", "Cash", "Chargeable", "Capital Work in Progress",
        ...
        "Receivable", "Round Off", "Round Off for Opening", "Stock",
        "Stock Adjustment", "Stock Received But Not Billed",
...
def sync_account(quickbooks_obj):
    ...
    tracker = ImportTracker(len(all_accounts), "qb_account_import.log", module_name="ACCOUNT", company=qb_company)

    stats = sync_qb_accounts(all_accounts, quickbooks_account_list, qb_company, tracker=tracker)
    _dbg("sync_account:done", {
        "created": len(quickbooks_account_list),
...
def sync_qb_accounts(get_qb_account, quickbooks_account_list, qb_company=None, tracker=None):
    ...
    if not tracker:
        from quickbooks_master_sync.quickbooks_master_sync.utils.import_tracker import ImportTracker
        tracker = ImportTracker(len(get_qb_account), "qb_account_import.log", module_name="ACCOUNT", company=resolved_company)
    ...
    for idx, qb_account in enumerate(get_qb_account):
        ...
        account_display_name = qb_account.get("Name") or qb_account.get("FullyQualifiedName") or qb_account.get("name") or "Unknown"
        ...
        if not exists:
            ...
            try:
                ...
                _dbg("sync_qb_accounts:create_failed", {
                    "idx": idx,
                    "Id": account_id,
                    "name": account_name,
                    "error": str(e)
                ...
                qb_log_error(
                    ...
                    message=_(
                        "Failed to create account '{0}' (QuickBooks ID: {1}). "
                        "Error: {2}"
                    ).format(account_name, account_id, str(e)),
                    module="sync_account",
...
def create_account(qb_account, quickbooks_account_list, qb_company=None):
    ...
    if existing_by_qb_id:
        _dbg("create_account:skip_exists_by_qb_id", {
    ...
    _dbg("create_account:mapping_result", {
        "qb_type": qb_account.get("AccountType"),
        "qb_subtype": qb_account.get("AccountSubType"),
        "root_type": root_type,
    ...
    try:
        ...
        _dbg("create_account:parent_error", {
            "error": str(e),
            "root_type": root_type,
            "account_name": qb_account.get("Name")
        })
    ...
    if existing_by_name and not existing_by_name.get("quickbooks_account_id"):
        ...
        try:
            frappe.db.set_value("Account", existing_by_name.get("name"), "quickbooks_account_id", unique_qb_id)
            # Note: Commit will be batched in sync_qb_accounts() for better performance
            quickbooks_account_list.append(str(qb_account.get("Id")))
            _dbg("create_account:linked_existing", {
                "account": existing_by_name.get("name"),
                "qb_id": qb_account.get("Id")
            })
            return quickbooks_account_list
        except Exception:
        ...
        _dbg("create_account:renamed_duplicate", {
    ...
    try:
        ...
        if account_number:
            ...
            if existing_with_acct_num:
                ...
                if existing_qb_id == unique_qb_id:
                    ...
                    _dbg("create_account:duplicate_account_number", {
        ...
        try:
            ...
            _dbg("create_account:retry_duplicate_name", {
