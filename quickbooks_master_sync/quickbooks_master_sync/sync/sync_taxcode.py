...
def sync_tax_code(quickbooks_obj):
	...
	try:
		...
		frappe.throw(_("Quickbooks Settings not found. Please configure Quickbooks Settings first."))
	...
	try:
		...
		get_qb_tax_code = query_with_pagination(
			quickbooks_obj, tax_code_query, "TaxCode", module_name="sync_taxcode", data_name="tax_codes_list"
		)
		...
		stats = sync_qb_tax_codes(
			get_qb_tax_code, quickbooks_settings, _get_quickbooks_company(quickbooks_obj)
		)
...
def sync_qb_tax_codes(get_qb_tax_code, quickbooks_settings, company=None):
	...
	tracker = ImportTracker(
		len(get_qb_tax_code), "qb_tax_code_import.log", company=company, module_name="TAX_CODE"
	)
		...
		try:
			...
			if company:
				...
				existing_tax = frappe.db.get_value(
					...
					{"parent": "Quickbooks Settings", "quickbooks_tax_id": tax_code_id, "company": company},
				...
				existing_tax = frappe.db.sql(
                    ...
                    LIMIT 1
                """,
					(quickbooks_settings.name, tax_code_id),
					as_dict=True,
				)
			...
			if existing_tax:
				...
				if (
					not sales_tax_rate_id
					and purchase_tax_rate_id
					and hasattr(existing_doc, "quickbooks_tax_rate_id")
				):
					if existing_doc.quickbooks_tax_rate_id != purchase_tax_rate_id:
						existing_doc.quickbooks_tax_rate_id = purchase_tax_rate_id
						updated = True
				if (
					not sales_tax_rate_name
					and purchase_tax_rate_name
					and hasattr(existing_doc, "quickbooks_tax_rate_name")
				):
			...
			stats["errors"] += 1
			stats["failed_tax_codes"].append({"qb_id": tax_code_id, "name": tax_code_name, "error": str(e)})
			tracker.log_error("QuickBooks", tracker_id, str(e))
			_dbg(
				"sync_qb_tax_codes:error",
				{"idx": idx, "tax_code_id": tax_code_id, "name": tax_code_name, "error": str(e)},
			)
			qb_log_error(
				title=_("Failed to Sync Tax Code"),
				status="Error",
				method="sync_qb_tax_codes",
				message=_("Failed to sync tax code '{0}' (QuickBooks ID: {1}). " "Error: {2}").format(
					tax_code_name, tax_code_id, str(e)
				),
				module="sync_taxcode",
	...
	if new_tax_codes:
		...
		for new_tax_code in new_tax_codes:
			...
			if batch_key in seen_in_batch:
				_dbg("sync_qb_tax_codes:skip_duplicate_in_batch", {"tax_id": tax_id, "company": company})
				stats["skipped"] += 1
				continue
			...
			if company is None:
				...
				if global_exists:
					_dbg(
						"sync_qb_tax_codes:skip_duplicate_global",
						{"tax_id": tax_id, "note": "Skipping - global mapping already exists"},
					)
					stats["skipped"] += 1
					continue
			else:
				...
				if (tax_id, company) in existing_tax_ids_with_company:
					...
					_dbg("sync_qb_tax_codes:skip_duplicate_company", {"tax_id": tax_id, "company": company})
					stats["skipped"] += 1
					continue
		...
		if filtered_new_tax_codes:
			...
			try:
				...
				if "duplicate" in error_msg.lower() or "already mapped" in error_msg.lower():
					_dbg(
						"sync_qb_tax_codes:save_validation_error",
						{
							"error": error_msg,
					...
					for new_tax_code in filtered_new_tax_codes:
						try:
							...
							_dbg(
								"sync_qb_tax_codes:skip_individual_duplicate",
								{
									"tax_id": new_tax_code.get("quickbooks_tax_id"),
...
def sync_tax_rate(quickbooks_obj):
	...
	try:
		...
		get_qb_tax_rate = query_with_pagination(
			quickbooks_obj, tax_rate_query, "TaxRate", module_name="sync_taxcode", data_name="tax_rates_list"
		)
...
def sync_qb_tax_rates(get_qb_tax_rate, quickbooks_settings, company):
	...
	tracker = ImportTracker(
		len(get_qb_tax_rate), "qb_tax_rate_import.log", company=company, module_name="TAX_RATE"
	)
	...
	for idx, qb_tax_rate in enumerate(get_qb_tax_rate):
		try:
            ...
            """,
				(quickbooks_settings.name, tax_rate_id, tax_rate_id),
				as_dict=True,
			)
			...
			if existing_taxes:
				...
				if len(existing_taxes) > 1:
					_dbg(
						"sync_qb_tax_rates:multiple_entries_found",
						{
			...
			if existing_taxes:
				...
				for tax_entry in existing_taxes:
					...
					tax_entry.get("quickbooks_tax_id")
					tax_entry.get("company")
					...
					try:
						...
						is_purchase_tax = (
							hasattr(existing_doc, "quickbooks_purchase_tax_rate_id")
							and existing_doc.quickbooks_purchase_tax_rate_id == tax_rate_id
						)
						is_sales_tax = (
							hasattr(existing_doc, "quickbooks_tax_rate_id")
							and existing_doc.quickbooks_tax_rate_id == tax_rate_id
						)
						...
						if updated:
							...
							if existing_doc.company:
                                ...
                                """,
									(
										quickbooks_settings.name,
										existing_doc.quickbooks_tax_id,
										existing_doc.name,
									),
									as_dict=True,
								)
							...
							if duplicate_check:
								...
								_dbg(
									"sync_qb_tax_rates:duplicate_skip",
									{
										"idx": idx,
										"tax_rate_id": tax_rate_id,
						...
						if auto_create and rate_value > 0 and tax_entry == existing_taxes[0]:
							creation_stats = auto_create_tax_objects(qb_tax_rate, company, tax_rate_id)
...
def create_item_tax_template(qb_tax_rate, company, tax_account):
	...
	try:
		...
		try:
				...
				if meta.has_field(fieldname):
					...
					if field and field.reqd:
						...
						default_service_code = (
							frappe.db.get_value("Company", company, fieldname)
							or frappe.db.get_value("Company", company, "custom_service_code")
							or ""
						)
			...
			for fieldname in cost_center_fields:
				if meta.has_field(fieldname):
					...
					if field and field.reqd:
						...
						default_cost_center = (
							frappe.db.get_value("Company", company, fieldname)
							or frappe.db.get_value("Company", company, "custom_cost_center")
			...
			_dbg("create_item_tax_template:field_check_error", {"error": str(field_check_error)})
		...
		_dbg("create_item_tax_template:error", {"error": str(e), "tax_name": qb_tax_rate.get("Name")})
		# Log the error but don't fail the sync
		frappe.log_error(
			title="Item Tax Template Creation Failed",
			message=f"Failed to create Item Tax Template for {tax_rate_name}: {e!s}",
		)
		return None
...
def create_tax_withholding_category(qb_tax_rate, company, tax_account):
	...
	try:
		...
		_dbg("create_withholding:error", {"error": str(e), "tax_name": qb_tax_rate.get("Name")})
		return None
...
def auto_create_tax_objects(qb_tax_rate, company, tax_rate_id):
	...
	try:
		...
		if tax_type == "item_tax":
			...
			if result:
				stats["templates"] = 1
				_dbg("auto_create:item_tax_created", {"tax_id": tax_rate_id, "template": result})

		elif tax_type == "withholding":
			result = create_tax_withholding_category(qb_tax_rate, company, tax_account)
			if result:
				stats["withholding"] = 1
				_dbg("auto_create:withholding_created", {"tax_id": tax_rate_id, "category": result})
