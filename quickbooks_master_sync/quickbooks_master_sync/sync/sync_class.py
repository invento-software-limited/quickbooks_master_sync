...
def sync_class(quickbooks_obj, qb_company=None):
    ...
    class_list = query_with_pagination(
        ...
        class_query,
        "Class",
        module_name="sync_class",
        data_name="class_list"
    )
    ...
    # Log summary
    summary_msg = _(
        "Class sync completed. Created: {0}, Updated: {1}, Skipped: {2}, Failed: {3}"
    ).format(
        stats.get("created", 0),
        stats.get("updated", 0),
        stats.get("skipped", 0),
...
def _sync_single_class(qb_class, qb_company, quickbooks_obj):
    ...
    _dbg("sync_single_class:processing", {
        "class_id": class_id,
    ...
    existing_cc = frappe.db.get_value(
        "Cost Center",
        {"quickbooks_class_id": unique_class_id, "company": qb_company},
        "name"
    )
    ...
    if parent_ref and parent_ref.get("value"):
        ...
        parent_cost_center = frappe.db.get_value(
            "Cost Center",
            {"quickbooks_class_id": unique_parent_id, "company": qb_company},
            "name"
        )
        
        if not parent_cost_center:
            _dbg("sync_single_class:parent_not_found", {
                "class_id": class_id,
    ...
    if not parent_cost_center:
        ...
        if not parent_cost_center:
            parent_cost_center = frappe.db.get_value(
                "Cost Center",
                {"company": qb_company, "cost_center_name": qb_company},
                "name"
            )
    ...
    if existing_cc:
        ...
        if changed:
            doc.save()
            _dbg("sync_single_class:updated", {
                "class_id": class_id,
                "cost_center": doc.name,
                "changes": "Updated cost center fields"
            })
            return "updated"
        else:
            _dbg("sync_single_class:skipped", {
                "class_id": class_id,
                "cost_center": doc.name,
        ...
        _dbg("sync_single_class:created", {
            "class_id": class_id,
            "cost_center": doc.name,
