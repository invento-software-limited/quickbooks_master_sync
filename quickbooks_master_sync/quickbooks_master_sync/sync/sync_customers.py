...
def sync_customers(quickbooks_obj):
    ...
    sync_qb_customers(
        get_qb_customer, quickbooks_customer_list, stats, qb_company, tracker=tracker
    )
    _dbg(
        "sync_customers:done",
        {
    ...
    if stats.get("errors", 0) > 0:
        ...
        if failed_customers:
            failed_names = [
                cust.get("name", "Unknown") for cust in failed_customers[:10]
            ]
            error_msg += _("\n\nFailed customers (first 10): {0}").format(
                ", ".join(failed_names)
            )
...
def sync_qb_customers(
    get_qb_customer, quickbooks_customer_list, stats=None, qb_company=None, tracker=None
):
    if stats is None:
        stats = {
            "created": 0,
    ...
    for idx, qb_customer in enumerate(get_qb_customer):
        ...
        if not existing_name:
            ...
            try:
                result = create_customer(
                    qb_customer, quickbooks_customer_list, qb_company
                )
                ...
                if result:
                    ...
                    stats["skipped"] += 1
                    tracker.log_skip(
                        "QuickBooks", tracker_id, "creation_failed_or_skipped"
                    )
                    _dbg(
                ...
                qb_log_error(
                    ...
                    message=_(
                        "Failed to create customer '{0}' (QuickBooks ID: {1}). "
                        "Error: {2}"
                    ).format(customer_name, qb_id, str(e)),
                    module="sync_customers",
            ...
            try:
                ...
                qb_log_error(
                    ...
                    method="sync_qb_customers",
                    message=_(
                        "Failed to update customer '{0}' (QuickBooks ID: {1}). "
                        "Error: {2}"
                    ).format(customer_name, qb_id, str(e)),
                    module="sync_customers",
                    request_data=qb_customer,
        ...
        if (
            current_processed > 0
            and (current_processed - last_commit_count) >= commit_interval
        ):
            frappe.db.commit()
...
def create_customer(qb_customer, quickbooks_customer_list, qb_company=None):
    ...
    try:
        ...
        if is_deleted and not customer_name.lower().endswith("(deleted)"):
            ...
            customer_name = (
                customer_name.replace(" (deleted)", "").replace("(deleted)", "").strip()
            )
        ...
        if existing_customer:
            ...
            existing_qb_id = frappe.db.get_value(
                "Customer", existing_customer, "quickbooks_cust_id"
            )
            if not existing_qb_id:
                ...
                update_customer(
                    qb_customer, existing_customer, quickbooks_customer_list
                )
                return True  # Successfully updated
            else:
        ...
        if qb_customer.get("PrimaryPhone"):
            if isinstance(qb_customer.get("PrimaryPhone"), dict):
                primary_phone = qb_customer["PrimaryPhone"].get("FreeFormNumber", "")
            else:
                primary_phone = str(qb_customer.get("PrimaryPhone"))
        if qb_customer.get("Mobile"):
            if isinstance(qb_customer.get("Mobile"), dict):
                mobile_phone = qb_customer["Mobile"].get("FreeFormNumber", "")
            else:
                mobile_phone = str(qb_customer.get("Mobile"))
        ...
        if qb_customer.get("Job"):
            ...
            customer_type = _(
                "Company"
            )  # or could be "Individual" based on business logic
        ...
        payload = {
            ...
            "default_currency": (
                qb_customer["CurrencyRef"].get("value", "")
            ...
            "custom_onboarding_date": _get_default_for_customer_field(
                "custom_onboarding_date"
            ),
        ...
        try:
            ...
            if duplicate_name:
                ...
                existing_qb_id = frappe.db.get_value(
                    "Customer", duplicate_name, "quickbooks_cust_id"
                )
                ...
                if not existing_qb_id:
                    ...
                    update_customer(
                        qb_customer, duplicate_name, quickbooks_customer_list
                    )
                    return True  # Successfully updated
                else:
        ...
        if qb_customer.get("Active") is not None:
            try:
                ...
                if meta.has_field("disabled"):
                    ...
                    is_deleted = (
                        "(deleted)" in qb_display_name.lower()
                        if qb_display_name
                        else False
                    )
        ...
        if qb_customer.get("Fax"):
            ...
            if fax_number:
                try:
                    ...
                    if meta.has_field("custom_binvat_registration_no"):
                        if (
                            getattr(customer, "custom_binvat_registration_no", None)
                            != fax_number
                        ):
            ...
            if term_name:
                try:
                    ...
                    if meta.has_field("payment_terms_template"):
                        ...
                        if payment_term:
                            if (
                                getattr(customer, "payment_terms_template", None)
                                != payment_term
                            ):
        ...
        try:
            ...
            if "frappe.utils.modules" in str(e):
                ...
                modules_to_clear = [
                    ...
                    if "crm.api" in k or "frappe.utils.modules" in k
                ]
...
def update_customer_address(customer, address, address_type="Billing"):
    ...
    try:
        ...
        unique_address_id = make_unique_qb_id(
            str(address.get("Id")), customer.custom_company
        )
...
def create_customer_address(customer, address, address_type="Billing"):
    address_title, resolved_address_type = get_address_title_and_type(
        customer.customer_name, address_type
    )
    try:
        # Build address lines
        ...
        unique_address_id = (
            make_unique_qb_id(str(address.get("Id")), customer.company)
            if address.get("Id")
            else None
        )
...
def create_customer_contact(customer, qb_customer):
    ...
    try:
        ...
        if phone or mobile_no:
            ...
            if mobile_no:
                contact_payload["phone_nos"].append(
...
def erp_customer_data():
    erp_customer = frappe.db.sql(
        (
            ...
            "WHERE  quickbooks_cust_id IS NULL"
        ),
        as_dict=1,
    )
    return erp_customer
...
def get_all_customers_json(
    ...
    if source == "erpnext":
        ...
        return get_quickbooks_customers_json(
...
def get_erpnext_customers_json(include_addresses=True):
    ...
    try:
        ...
        customers = frappe.get_all(
            "Customer", fields=fields_to_query, order_by="customer_name"
        )
                ...
                for field in meta.fields:
                    if field.fieldname.startswith("custom_"):
                        if hasattr(doc, field.fieldname):
                            customer_dict[field.fieldname] = getattr(
...
def get_customers_json_api(source="erpnext", include_addresses=True):
    ...
    try:
        if source == "quickbooks":
            ...
            json_data = get_quickbooks_customers_json(
                quickbooks_obj, include_addresses=include_addresses
            )
        else:
        ...
        frappe.log_error(
            f"Error getting customers JSON: {str(e)}", "get_customers_json_api"
        )
        return {
            "success": False,
            "error": str(e),
