...
def sync_payment_method(quickbooks_obj):
    ...
    tracker = ImportTracker(len(get_qb_payment_methods), "qb_payment_method_import.log", module_name="PAYMENT_METHOD", company=company)

    for qb_payment_method in get_qb_payment_methods:
        try:
            qb_id = str(qb_payment_method.get("Id", ""))
            qb_name = qb_payment_method.get("Name", "")
            qb_type = qb_payment_method.get("Type", "")
            qb_active = qb_payment_method.get("Active", True)
            ...
            if not qb_id or not qb_name:
                ...
                _dbg("sync_payment_method:skipped_invalid", {
            ...
            existing_mode = frappe.db.get_value(
                "Mode of Payment",
                {"quickbooks_payment_method_id": unique_qb_id},
                "name",
                as_dict=True
            )
            ...
            if existing_mode:
                ...
                try:
                    ...
                    frappe.db.commit()
                    stats["updated"] += 1
                    _dbg("sync_payment_method:updated", {
                        "qb_id": qb_id,
                        "name": qb_name,
                    ...
                    stats["failed"] += 1
                    _dbg("sync_payment_method:update_error", {
                        "qb_id": qb_id,
                        "error": str(e),
                    })
                    qb_log_exception(
                ...
                if existing_by_name:
                    ...
                    try:
                        ...
                        frappe.db.commit()
                        stats["updated"] += 1
                        _dbg("sync_payment_method:updated_by_name", {
                            "qb_id": qb_id,
                            "name": qb_name,
                        ...
                        stats["failed"] += 1
                        _dbg("sync_payment_method:update_by_name_error", {
                            "qb_id": qb_id,
                            "name": qb_name,
                            "error": str(e),
                        })
                    ...
                    try:
                        ...
                        frappe.db.commit()
                        stats["created"] += 1
                        _dbg("sync_payment_method:created", {
                            "qb_id": qb_id,
                            "name": qb_name,
                        ...
                        stats["failed"] += 1
                        _dbg("sync_payment_method:create_error", {
                            "qb_id": qb_id,
                            "name": qb_name,
                            "error": str(e),
                        })
            ...
            stats["failed"] += 1
            _dbg("sync_payment_method:error", {
                "error": str(e),
                "payment_method": qb_payment_method,
            })
            qb_log_exception(
            ...
            tracker.log_error("QuickBooks", f"{qb_payment_method.get('Id')} ({qb_payment_method.get('Name')})", str(e))
