...
def create_Employee(quickbooks_obj):
	...
	get_qb_employee = query_with_pagination(
		quickbooks_obj, employee_query, "Employee", module_name="sync_employee", data_name="employees_list"
	)
	...
	tracker = ImportTracker(
		len(get_qb_employee), "qb_employee_import.log", company=company, module_name="EMPLOYEE"
	)
	...
	try:
		for fields in get_qb_employee:
			...
			if not qb_employee_id:
				...
				_dbg("create_Employee:skip_no_id", {"employee": display_name})
				continue
			...
			if existing_employee:
				...
				_dbg(
					"create_Employee:already_exists",
					{
			...
			employee.date_of_joining = (
				fields.get("HiredDate") if fields.get("HiredDate") else strftime("%Y-%m-%d")
			)
			employee.date_of_birth = fields.get("BirthDate") if fields.get("BirthDate") else "1916-04-01"
			...
			employee.personal_email = (
				fields["PrimaryEmailAddr"].get("Address", "") if fields.get("PrimaryEmailAddr") else ""
			)
			...
			for response_obj in response_from_quickbooks.successes:
				if response_obj:
					...
					employee_company = frappe.db.get_value(
						"Employee", {"employee_name": response_obj.DisplayName}, "company"
					)
					if employee_company:
