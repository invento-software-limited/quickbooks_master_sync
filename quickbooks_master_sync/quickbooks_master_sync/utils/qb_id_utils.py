...
def make_unique_qb_id(qb_id, company, prefix=None):
    ...
    try:
        ...
        if not abbr:
            ...
            frappe.log_error(
                f"Company abbreviation not found for: {company}",
                "make_unique_qb_id"
            )
            if prefix:
                 return f"{prefix}{qb_id}"
        ...
        frappe.log_error(
            f"Error creating unique QB ID for {qb_id} and company {company}: {str(e)}",
            "make_unique_qb_id"
        )
        return str(qb_id)
...
def migrate_qb_id(doctype, qb_id_field, company_field="company"):
    ...
    try:
        ...
        records = frappe.get_all(
            ...
            filters={
                qb_id_field: ["!=", ""],
                qb_id_field: ["not like", "%-%"]  # Don't have company prefix yet
            },
        ...
        frappe.log_error(
            f"Error migrating QB IDs for {doctype}: {str(e)}",
            "migrate_qb_id"
        )
        raise
